...
const DATA_DIR = pick('DATA_DIR', path.join(PROJECT_ROOT, 'data'));
...
const PROMPTS_DIR = pick('PROMPTS_DIR', path.join(PROJECT_ROOT, 'prompts'));
...
function pickLiteBudgetNum(key, normalFallback, liteCap) {
  const value = pickNum(key, lowResourceLiteBudget ? liteCap : normalFallback);
  if (!lowResourceLiteBudget) return value;
  const cap = Number(liteCap);
...
module.exports = {
  ...
  MIZUKIBOT_RUNTIME_ROLE: runtimeRole,
  LOW_RESOURCE_DISABLE_LANCEDB_HOT_PATH: lowResourceDisableLanceDbHotPath,
  LOW_RESOURCE_DISABLE_WORLDBOOK_SEMANTIC: lowResourceDisableWorldbookSemantic,
  LOW_RESOURCE_LITE_BUDGET: lowResourceLiteBudget,
  MAIN_REPLY_PROMPT_MODE: pick('MAIN_REPLY_PROMPT_MODE', 'balanced').toLowerCase(),
  LOCAL_PROMPT_RECALL_ENABLED: pickBool('LOCAL_PROMPT_RECALL_ENABLED', true),
  LOCAL_PROMPT_RECALL_DB_FILE: pick('LOCAL_PROMPT_RECALL_DB_FILE', path.join(DATA_DIR, 'local_prompt_recall.sqlite')),
  LOCAL_PROMPT_RECALL_TOP_EXAMPLES: Math.max(0, pickNum('LOCAL_PROMPT_RECALL_TOP_EXAMPLES', 2)),
  LOCAL_PROMPT_RECALL_TOP_MODULES: Math.max(0, pickNum('LOCAL_PROMPT_RECALL_TOP_MODULES', 2)),
  LOW_RESOURCE_LANCEDB_HELPER_ENABLED: pickBool('LOW_RESOURCE_LANCEDB_HELPER_ENABLED', lowResourceLiteBudget),
  LOW_RESOURCE_LANCEDB_HELPER_TIMEOUT_MS: Math.max(0, pickNum('LOW_RESOURCE_LANCEDB_HELPER_TIMEOUT_MS', lowResourceLiteBudget ? 2500 : 0)),
  ...
  PRIVATE_STATUS_BAR_ENABLED: pickBool('PRIVATE_STATUS_BAR_ENABLED', false),
  PRIVATE_STATUS_BAR_API_BASE_URL: pick('PRIVATE_STATUS_BAR_API_BASE_URL', ''),
  PRIVATE_STATUS_BAR_API_KEY: pick('PRIVATE_STATUS_BAR_API_KEY', ''),
  PRIVATE_STATUS_BAR_MODEL: pick('PRIVATE_STATUS_BAR_MODEL', ''),
  PRIVATE_STATUS_BAR_TIMEOUT_MS: Math.max(1000, pickNum('PRIVATE_STATUS_BAR_TIMEOUT_MS', 8000)),
  PRIVATE_STATUS_BAR_MAX_TOKENS: Math.max(256, pickNum('PRIVATE_STATUS_BAR_MAX_TOKENS', 25000)),
  PRIVATE_STATUS_BAR_IMAGE_URLS: pick('PRIVATE_STATUS_BAR_IMAGE_URLS', '{}'),
  LIVE2D_EMOTION_ENABLED: pickBool('LIVE2D_EMOTION_ENABLED', false),
  LIVE2D_EMOTION_MIN_CONFIDENCE: Math.min(1, Math.max(0, pickNum('LIVE2D_EMOTION_MIN_CONFIDENCE', 0.7))),
  LIVE2D_EMOTION_COOLDOWN_MS: Math.max(0, pickNum('LIVE2D_EMOTION_COOLDOWN_MS', 120000)),
  LIVE2D_EMOTION_MANIFEST: pick('LIVE2D_EMOTION_MANIFEST', path.join(PROJECT_ROOT, 'assets/live2d/emotions.json')),
  LIVE2D_MODEL_DIR: pick('LIVE2D_MODEL_DIR', path.join(PROJECT_ROOT, 'assets/live2d')),
  LIVE2D_RENDER_ENABLED: pickBool('LIVE2D_RENDER_ENABLED', false),
  LIVE2D_RENDER_TIMEOUT_MS: Math.max(1000, pickNum('LIVE2D_RENDER_TIMEOUT_MS', 10000)),
  LIVE2D_BROWSER_EXECUTABLE_PATH: pick('LIVE2D_BROWSER_EXECUTABLE_PATH', ''),
  LIVE2D_RENDER_WORKER_MODULE: pick('LIVE2D_RENDER_WORKER_MODULE', ''),
  SMALL_THEATER_ENABLED: pickBool('SMALL_THEATER_ENABLED', false),
  SMALL_THEATER_MAX_INPUT_CHARS: Math.max(1, pickNum('SMALL_THEATER_MAX_INPUT_CHARS', 4000)),
  SMALL_THEATER_MEMORY_TOP_K: Math.max(1, Math.min(20, pickNum('SMALL_THEATER_MEMORY_TOP_K', 8))),
  SMALL_THEATER_MODEL_TIMEOUT_MS: Math.max(1000, pickNum('SMALL_THEATER_MODEL_TIMEOUT_MS', 60000)),
  SMALL_THEATER_COOLDOWN_MS: Math.max(0, pickNum('SMALL_THEATER_COOLDOWN_MS', 15000)),
  SMALL_THEATER_FAILURE_RETRY_COOLDOWN_MS: Math.max(0, pickNum('SMALL_THEATER_FAILURE_RETRY_COOLDOWN_MS', 5000)),
  SMALL_THEATER_MAX_CONCURRENCY: Math.max(1, pickNum('SMALL_THEATER_MAX_CONCURRENCY', 2)),
  NAPCAT_WS_URL: pick('NAPCAT_WS_URL', ''),
  NAPCAT_WS_TOKEN: pick('NAPCAT_WS_TOKEN', ''),
  // HTTP reverse ingress port (NapCat posts events to bot)
  NAPCAT_HTTP_REVERSE_ENABLED: pickBool('NAPCAT_HTTP_REVERSE_ENABLED', true),
  ...
  NAPCAT_HTTP_REVERSE_ALLOW_LEGACY_BEARER: pickBool('NAPCAT_HTTP_REVERSE_ALLOW_LEGACY_BEARER', false),
  NAPCAT_HTTP_REVERSE_TRUST_PROXY: pick('NAPCAT_HTTP_REVERSE_TRUST_PROXY', 'loopback'),
  NAPCAT_HTTP_REVERSE_MAX_BODY_BYTES: Math.max(1024, pickNum('NAPCAT_HTTP_REVERSE_MAX_BODY_BYTES', 2 * 1024 * 1024)),
  NAPCAT_HTTP_REVERSE_SIGNATURE_MAX_AGE_MS: Math.max(1000, pickNum('NAPCAT_HTTP_REVERSE_SIGNATURE_MAX_AGE_MS', 5 * 60 * 1000)),
  NAPCAT_HTTP_REVERSE_RATE_LIMIT_WINDOW_MS: Math.max(1000, pickNum('NAPCAT_HTTP_REVERSE_RATE_LIMIT_WINDOW_MS', 60 * 1000)),
  NAPCAT_HTTP_REVERSE_RATE_LIMIT_MAX: Math.max(1, pickNum('NAPCAT_HTTP_REVERSE_RATE_LIMIT_MAX', 120)),
  PRIVATE_MESSAGE_RESTART_RECOVERY_ENABLED: pickBool('PRIVATE_MESSAGE_RESTART_RECOVERY_ENABLED', true),
  PRIVATE_MESSAGE_RESTART_RECOVERY_STATE_FILE: pick('PRIVATE_MESSAGE_RESTART_RECOVERY_STATE_FILE', path.join(DATA_DIR, 'private-message-restart-recovery.json')),
  PRIVATE_MESSAGE_RESTART_RECOVERY_LOOKBACK_MS: Math.max(60 * 1000, pickNum('PRIVATE_MESSAGE_RESTART_RECOVERY_LOOKBACK_MS', 6 * 60 * 60 * 1000)),
  PRIVATE_MESSAGE_RESTART_RECOVERY_OVERLAP_MS: Math.max(0, pickNum('PRIVATE_MESSAGE_RESTART_RECOVERY_OVERLAP_MS', 2 * 60 * 1000)),
  PRIVATE_MESSAGE_RESTART_RECOVERY_CONTACT_LIMIT: Math.max(1, pickNum('PRIVATE_MESSAGE_RESTART_RECOVERY_CONTACT_LIMIT', 100)),
  PRIVATE_MESSAGE_RESTART_RECOVERY_HISTORY_COUNT: Math.max(5, pickNum('PRIVATE_MESSAGE_RESTART_RECOVERY_HISTORY_COUNT', 30)),
  ...
  LUCKIN_MCP_TIMEOUT_MS: Math.max(1000, pickNum('LUCKIN_MCP_TIMEOUT_MS', 12000)),
  LUCKIN_LOCATION_TIMEOUT_MS: Math.max(1000, pickNum('LUCKIN_LOCATION_TIMEOUT_MS', 8000)),
  LUCKIN_SESSION_TTL_MS: Math.max(60000, pickNum('LUCKIN_SESSION_TTL_MS', 15 * 60 * 1000)),
  LUCKIN_USER_RATE_LIMIT_WINDOW_MS: Math.max(1000, pickNum('LUCKIN_USER_RATE_LIMIT_WINDOW_MS', 60 * 1000)),
  LUCKIN_USER_RATE_LIMIT_MAX: Math.max(1, pickNum('LUCKIN_USER_RATE_LIMIT_MAX', 12)),
  LUCKIN_MINIAPP_CARD_PAYLOAD: pick('LUCKIN_MINIAPP_CARD_PAYLOAD', ''),
  ...
  FOLLOWER_PACKET_LOG_ENABLED: pickBool('FOLLOWER_PACKET_LOG_ENABLED', false),
  ...
  QQ_THINKING_EMOJI_IDS: pickIntList('QQ_THINKING_EMOJI_IDS', [355]),
  QQ_THINKING_EMOJI_TIMEOUT_MS: Math.max(1000, pickNum('QQ_THINKING_EMOJI_TIMEOUT_MS', 3000)),
  QQ_SAFETY_RESTRICTION_EMOJI_IDS: pickIntList('QQ_SAFETY_RESTRICTION_EMOJI_IDS', [39]),
  ...
  PROACTIVE_GROUP_OUTBOUND_ENABLED: pickBool('PROACTIVE_GROUP_OUTBOUND_ENABLED', true),
  PRIVATE_PROACTIVE_ENABLED: pickBool('PRIVATE_PROACTIVE_ENABLED', true),
  PRIVATE_PROACTIVE_IDLE_MINUTES: Math.max(1, pickNum('PRIVATE_PROACTIVE_IDLE_MINUTES', 120)),
  PRIVATE_PROACTIVE_MIN_GAP_MINUTES: Math.max(1, pickNum('PRIVATE_PROACTIVE_MIN_GAP_MINUTES', 240)),
  PRIVATE_PROACTIVE_MAX_PER_DAY: Math.max(1, Math.floor(pickNum('PRIVATE_PROACTIVE_MAX_PER_DAY', 2))),
  PRIVATE_PROACTIVE_SCAN_INTERVAL_MINUTES: Math.max(1, pickNum('PRIVATE_PROACTIVE_SCAN_INTERVAL_MINUTES', 10)),
  PRIVATE_PROACTIVE_WINDOWS: pick('PRIVATE_PROACTIVE_WINDOWS', '09:00-15:00,17:00-23:00'),
  ...
  COMPANION_VOICE_ENABLED: pickBool('COMPANION_VOICE_ENABLED', false),
  COMPANION_VOICE_PROVIDER: pick('COMPANION_VOICE_PROVIDER', 'external').toLowerCase(),
  COMPANION_VOICE_API_URL: pick('COMPANION_VOICE_API_URL', ''),
  COMPANION_VOICE_API_KEY: pick('COMPANION_VOICE_API_KEY', ''),
  COMPANION_VOICE_MODEL: pick('COMPANION_VOICE_MODEL', 'gpt-4o-mini-tts'),
  COMPANION_VOICE_LOCAL_API_URL: pick('COMPANION_VOICE_LOCAL_API_URL', ''),
  COMPANION_VOICE_LOCAL_API_KEY: pick('COMPANION_VOICE_LOCAL_API_KEY', ''),
  COMPANION_VOICE_NAME: pick('COMPANION_VOICE_NAME', 'coral'),
  COMPANION_VOICE_SPEED: Math.max(0.25, Math.min(4, pickNum('COMPANION_VOICE_SPEED', 1))),
  COMPANION_VOICE_TIMEOUT_MS: Math.max(1000, pickNum('COMPANION_VOICE_TIMEOUT_MS', 15000)),
  COMPANION_VOICE_MAX_CHARS: Math.min(300, Math.max(1, pickNum('COMPANION_VOICE_MAX_CHARS', 300))),
  COMPANION_VOICE_MAX_SEGMENTS: Math.max(1, Math.floor(pickNum('COMPANION_VOICE_MAX_SEGMENTS', 4))),
  COMPANION_VOICE_MAX_CONCURRENCY: Math.max(1, Math.floor(pickNum('COMPANION_VOICE_MAX_CONCURRENCY', 2))),
  COMPANION_VOICE_WEIXIN_NATIVE_ENABLED: pickBool('COMPANION_VOICE_WEIXIN_NATIVE_ENABLED', false),
  COMPANION_VOICE_WEIXIN_FFMPEG_PATH: pick('COMPANION_VOICE_WEIXIN_FFMPEG_PATH', 'ffmpeg'),
  VOICE_INPUT_ENABLED: pickBool('VOICE_INPUT_ENABLED', false),
  VOICE_INPUT_API_URL: pick('VOICE_INPUT_API_URL', 'https://api.siliconflow.cn/v1/audio/transcriptions'),
  VOICE_INPUT_API_KEY: pick('VOICE_INPUT_API_KEY', ''),
  VOICE_INPUT_MODEL: pick('VOICE_INPUT_MODEL', 'XingChenAGI/XingChenASR-V3.2-Ultra'),
  VOICE_INPUT_TIMEOUT_MS: Math.max(1000, pickNum('VOICE_INPUT_TIMEOUT_MS', 60000)),
  VOICE_INPUT_MAX_BYTES: Math.max(1, pickNum('VOICE_INPUT_MAX_BYTES', 5 * 1024 * 1024)),
  VOICE_INPUT_MAX_CONCURRENCY: Math.max(1, Math.floor(pickNum('VOICE_INPUT_MAX_CONCURRENCY', 4))),
  ...
  COMPANION_ROOM_WEB_USER_ID: pick('COMPANION_ROOM_WEB_USER_ID', ''),
  WEATHER_ALERT_ENABLED: pickBool('WEATHER_ALERT_ENABLED', false),
  QWEATHER_API_KEY: pick('QWEATHER_API_KEY', ''),
  WEATHER_ALERT_SCAN_TIMES: pick('WEATHER_ALERT_SCAN_TIMES', '10:00,21:00'),
  WEATHER_ALERT_STATE_FILE: pick('WEATHER_ALERT_STATE_FILE', path.join(DATA_DIR, 'weather-alert-state.json')),
  EMAIL_GREETING_ENABLED: pickBool('EMAIL_GREETING_ENABLED', false),
  EMAIL_GREETING_STATE_FILE: pick('EMAIL_GREETING_STATE_FILE', path.join(DATA_DIR, 'email-greeting-state.json')),
  EMAIL_GREETING_SEND_TIME: pick('EMAIL_GREETING_SEND_TIME', '09:00'),
  EMAIL_GREETING_SCAN_INTERVAL_MS: Math.max(60 * 1000, pickNum('EMAIL_GREETING_SCAN_INTERVAL_MS', 10 * 60 * 1000)),
  EMAIL_GREETING_VERIFICATION_TTL_MINUTES: Math.max(1, pickNum('EMAIL_GREETING_VERIFICATION_TTL_MINUTES', 15)),
  EMAIL_GREETING_SMTP_HOST: pick('EMAIL_GREETING_SMTP_HOST', ''),
  EMAIL_GREETING_SMTP_PORT: Math.max(1, pickNum('EMAIL_GREETING_SMTP_PORT', 587)),
  EMAIL_GREETING_SMTP_SECURE: pickBool('EMAIL_GREETING_SMTP_SECURE', false),
  EMAIL_GREETING_SMTP_USER: pick('EMAIL_GREETING_SMTP_USER', ''),
  EMAIL_GREETING_SMTP_PASS: pick('EMAIL_GREETING_SMTP_PASS', ''),
  EMAIL_GREETING_SMTP_FROM: pick('EMAIL_GREETING_SMTP_FROM', ''),
  ...
  WEB_REQUIRE_HTTPS: pickBool('WEB_REQUIRE_HTTPS', true),
  ...
  WEB_LOGIN_RATE_LIMIT_STATE_FILE: pick('WEB_LOGIN_RATE_LIMIT_STATE_FILE', path.join(DATA_DIR, 'web-login-rate-limit.json')),
  WEB_VIEWER_TOKEN: pick('WEB_VIEWER_TOKEN', ''),
  WEB_AUDIT_LOG_FILE: pick('WEB_AUDIT_LOG_FILE', path.join(DATA_DIR, 'web-audit.jsonl')),
  ...
  MODEL_TLS_IMPERSONATION_ENABLED: pickBool('MODEL_TLS_IMPERSONATION_ENABLED', false),
  MODEL_TLS_IMPERSONATION_STREAM_ENABLED: pickBool('MODEL_TLS_IMPERSONATION_STREAM_ENABLED', false),
  MODEL_TLS_IMPERSONATION_FALLBACK_ENABLED: pickBool('MODEL_TLS_IMPERSONATION_FALLBACK_ENABLED', true),
  MODEL_TLS_IMPERSONATION_CLIENT: pick('MODEL_TLS_IMPERSONATION_CLIENT', 'chrome'),
  MODEL_TLS_IMPERSONATION_JA3: pick('MODEL_TLS_IMPERSONATION_JA3', CHROME_LIKE_JA3),
  MODEL_TLS_IMPERSONATION_JA4R: pick('MODEL_TLS_IMPERSONATION_JA4R', ''),
  MODEL_TLS_IMPERSONATION_HTTP2: pick('MODEL_TLS_IMPERSONATION_HTTP2', '1:65536;2:0;4:6291456;6:262144|15663105|0|m,a,s,p'),
  MODEL_TLS_IMPERSONATION_CONNECTION_REUSE_ENABLED: pickBool('MODEL_TLS_IMPERSONATION_CONNECTION_REUSE_ENABLED', false),
  MODEL_TLS_IMPERSONATION_FORCE_HTTP1: pickBool('MODEL_TLS_IMPERSONATION_FORCE_HTTP1', false),
  MODEL_TLS_IMPERSONATION_INSECURE_SKIP_VERIFY: pickBool('MODEL_TLS_IMPERSONATION_INSECURE_SKIP_VERIFY', false),
  ...
  LOCAL_COMMAND_BRIDGE_ENABLED: pickBool('LOCAL_COMMAND_BRIDGE_ENABLED', false),
  ...
  MESSAGE_INGRESS_ASYNC_ENABLED: pickBool('MESSAGE_INGRESS_ASYNC_ENABLED', true),
  MESSAGE_INGRESS_ASYNC_MAX_ACTIVE: Math.max(1, pickNum('MESSAGE_INGRESS_ASYNC_MAX_ACTIVE', 64)),
  MESSAGE_INGRESS_ASYNC_SHUTDOWN_DRAIN_MS: Math.max(0, pickNum('MESSAGE_INGRESS_ASYNC_SHUTDOWN_DRAIN_MS', 15000)),
  RUNTIME_SHUTDOWN_TIMEOUT_MS: Math.max(1000, pickNum('RUNTIME_SHUTDOWN_TIMEOUT_MS', 15000)),
  BOT_WORKER_THREADS_ENABLED: pickBool('BOT_WORKER_THREADS_ENABLED', true),
  BOT_WORKER_THREADS_MAX: Math.max(1, pickNum('BOT_WORKER_THREADS_MAX', 2)),
  BOT_WORKER_THREADS_QUEUE_MAX: Math.max(0, pickNum('BOT_WORKER_THREADS_QUEUE_MAX', 100)),
  BOT_WORKER_THREADS_TASK_TIMEOUT_MS: Math.max(1000, pickNum('BOT_WORKER_THREADS_TASK_TIMEOUT_MS', 120000)),
  POST_REPLY_QUEUE_DIR: pick('POST_REPLY_QUEUE_DIR', path.join(DATA_DIR, 'post_reply_jobs')),
  POST_REPLY_TRACE_DIR: pick('POST_REPLY_TRACE_DIR', path.join(DATA_DIR, 'post_reply_traces')),
  REQUEST_TRACE_HASH_SECRET: pick('REQUEST_TRACE_HASH_SECRET', ''),
  ...
  POST_REPLY_WORKER_SUPERVISOR_ENABLED: pickBool('POST_REPLY_WORKER_SUPERVISOR_ENABLED', true),
  POST_REPLY_WORKER_POLL_MS: pickNum('POST_REPLY_WORKER_POLL_MS', 2000),
  POST_REPLY_WORKER_CONCURRENCY: Math.max(1, pickNum('POST_REPLY_WORKER_CONCURRENCY', 2)),
  POST_REPLY_WORKER_PRESSURE_MAX_CONCURRENCY: Math.max(1, pickNum('POST_REPLY_WORKER_PRESSURE_MAX_CONCURRENCY', 1)),
  POST_REPLY_WORKER_SUPERVISOR_COOLDOWN_MS: Math.max(0, pickNum('POST_REPLY_WORKER_SUPERVISOR_COOLDOWN_MS', 30000)),
  POST_REPLY_WORKER_IDLE_RECYCLE_ENABLED: pickBool('POST_REPLY_WORKER_IDLE_RECYCLE_ENABLED', false),
  POST_REPLY_WORKER_RSS_RECYCLE_MB: Math.max(0, pickNum('POST_REPLY_WORKER_RSS_RECYCLE_MB', lowResourceMode ? 768 : 0)),
  ...
  POST_REPLY_QUEUE_LOCK_TIMEOUT_MS: Math.max(1, pickNum('POST_REPLY_QUEUE_LOCK_TIMEOUT_MS', 5000)),
  POST_REPLY_QUEUE_STALE_LOCK_MS: Math.max(1000, pickNum('POST_REPLY_QUEUE_STALE_LOCK_MS', 30000)),
  ...
  ...buildPostReplyWatchdogRuntimeConfig({ pick, pickNum, pickBool }),
  ...
  POST_REPLY_ENRICH_MAX_TURNS: Math.max(1, pickNum('POST_REPLY_ENRICH_MAX_TURNS', 12)),
  POST_REPLY_ENRICH_MAX_CHARS: Math.max(200, pickNum('POST_REPLY_ENRICH_MAX_CHARS', 6000)),
  POST_REPLY_ENRICH_MAX_WRITES: Math.max(1, pickNum('POST_REPLY_ENRICH_MAX_WRITES', 12)),
  POST_REPLY_ENRICH_PRESSURE_PAUSE_ENABLED: pickBool('POST_REPLY_ENRICH_PRESSURE_PAUSE_ENABLED', true),
  POST_REPLY_CORE_MINIMAL_UNDER_PRESSURE: pickBool('POST_REPLY_CORE_MINIMAL_UNDER_PRESSURE', true),
  POST_REPLY_MEMORY_MODE: pick('POST_REPLY_MEMORY_MODE', 'core').toLowerCase(),
  POST_REPLY_EXPLICIT_MEMORY_BYPASS_GROUP_ALLOWLIST: pickBool('POST_REPLY_EXPLICIT_MEMORY_BYPASS_GROUP_ALLOWLIST', false),
  ...
  POST_REPLY_AUTO_REQUEUE_TRANSIENT_ENABLED: pickBool('POST_REPLY_AUTO_REQUEUE_TRANSIENT_ENABLED', pickBool('POST_REPLY_FAILED_TRANSIENT_REQUEUE_ENABLED', false)),
  POST_REPLY_AUTO_REQUEUE_MAX_PER_TICK: Math.max(1, pickNum('POST_REPLY_AUTO_REQUEUE_MAX_PER_TICK', 3)),
  POST_REPLY_FAILED_TRANSIENT_REQUEUE_ENABLED: pickBool('POST_REPLY_FAILED_TRANSIENT_REQUEUE_ENABLED', pickBool('POST_REPLY_AUTO_REQUEUE_TRANSIENT_ENABLED', false)),
  INBOUND_GLOBAL_MAX_CONCURRENCY: Math.max(1, pickNum('INBOUND_GLOBAL_MAX_CONCURRENCY', 16)),
  INBOUND_GENERAL_MAX_CONCURRENCY: Math.max(1, pickNum('INBOUND_GENERAL_MAX_CONCURRENCY', 16)),
  ...
  PRIVATE_INBOUND_GLOBAL_MAX_CONCURRENCY: Math.max(0, pickNum('PRIVATE_INBOUND_GLOBAL_MAX_CONCURRENCY', 16)),
  PRIVATE_INBOUND_GENERAL_MAX_CONCURRENCY: Math.max(0, pickNum('PRIVATE_INBOUND_GENERAL_MAX_CONCURRENCY', 16)),
  ...
  ANTHROPIC_ADAPTIVE_THINKING_ENABLED: pickBool('ANTHROPIC_ADAPTIVE_THINKING_ENABLED', false),
  MAIN_MODEL_ANTHROPIC_WEB_SEARCH_ENABLED: pickBool('MAIN_MODEL_ANTHROPIC_WEB_SEARCH_ENABLED', true),
  MAIN_MODEL_ANTHROPIC_WEB_SEARCH_MAX_USES: Math.max(1, Math.floor(pickNum('MAIN_MODEL_ANTHROPIC_WEB_SEARCH_MAX_USES', 2))),
  MAIN_MODEL_ANTHROPIC_WEB_SEARCH_ALLOWED_DOMAINS: pickList('MAIN_MODEL_ANTHROPIC_WEB_SEARCH_ALLOWED_DOMAINS', []),
  MAIN_MODEL_ANTHROPIC_WEB_SEARCH_BLOCKED_DOMAINS: pickList('MAIN_MODEL_ANTHROPIC_WEB_SEARCH_BLOCKED_DOMAINS', []),
  MAIN_MODEL_ANTHROPIC_WEB_SEARCH_LOCATION_CITY: pick('MAIN_MODEL_ANTHROPIC_WEB_SEARCH_LOCATION_CITY', ''),
  MAIN_MODEL_ANTHROPIC_WEB_SEARCH_LOCATION_REGION: pick('MAIN_MODEL_ANTHROPIC_WEB_SEARCH_LOCATION_REGION', ''),
  MAIN_MODEL_ANTHROPIC_WEB_SEARCH_LOCATION_COUNTRY: pick('MAIN_MODEL_ANTHROPIC_WEB_SEARCH_LOCATION_COUNTRY', ''),
  MAIN_MODEL_ANTHROPIC_WEB_SEARCH_LOCATION_TIMEZONE: pick('MAIN_MODEL_ANTHROPIC_WEB_SEARCH_LOCATION_TIMEZONE', ''),
  ...
  ADMIN_PRIVATE_MAIN_REPLY_STREAM_FIRST_TOKEN_TIMEOUT_MS: Math.max(0, pickNum('ADMIN_PRIVATE_MAIN_REPLY_STREAM_FIRST_TOKEN_TIMEOUT_MS', 75000)),
  ADMIN_PRIVATE_MAIN_REPLY_STREAM_TOTAL_TIMEOUT_MS: Math.max(0, pickNum('ADMIN_PRIVATE_MAIN_REPLY_STREAM_TOTAL_TIMEOUT_MS', 75000)),
  ...
  CREATE_AGENT_AFFECTION_THRESHOLD: Math.max(0, Math.min(100, pickNum('CREATE_AGENT_AFFECTION_THRESHOLD', 30))),
  ...
  IMAGE_MODEL_RETRIES: Math.max(0, Math.min(3, pickNum('IMAGE_MODEL_RETRIES', 3))),
  IMAGE_MODEL_INPUT_TOKEN_WARN_THRESHOLD: Math.max(1024, pickNum('IMAGE_MODEL_INPUT_TOKEN_WARN_THRESHOLD', 18000)),
  IMAGE_MODEL_INPUT_TOKEN_HARD_LIMIT: Math.max(2048, pickNum('IMAGE_MODEL_INPUT_TOKEN_HARD_LIMIT', 20000)),
  VISION_ROUTE_USER_TEXT_MAX_TOKENS: Math.max(256, pickNum('VISION_ROUTE_USER_TEXT_MAX_TOKENS', 6000)),
  VISION_ROUTE_SYSTEM_CONTEXT_MAX_TOKENS: Math.max(512, pickNum('VISION_ROUTE_SYSTEM_CONTEXT_MAX_TOKENS', 10000)),
  ...
  MAIN_REPLY_INPUT_TOKEN_WARN_THRESHOLD: pickNum('MAIN_REPLY_INPUT_TOKEN_WARN_THRESHOLD', 50000),
  MAIN_REPLY_INPUT_TOKEN_HARD_LIMIT: pickNum('MAIN_REPLY_INPUT_TOKEN_HARD_LIMIT', 100000),
  AI_RETRIES: pickNum('AI_RETRIES', 2),
  NORMAL_USER_MODEL_DAILY_LIMIT_ENABLED: pickBool('NORMAL_USER_MODEL_DAILY_LIMIT_ENABLED', true),
  NORMAL_USER_MODEL_DAILY_LIMIT: Math.max(0, pickNum('NORMAL_USER_MODEL_DAILY_LIMIT', 25)),
  NORMAL_USER_MODEL_DAILY_LIMIT_STATE_FILE: pick(
    'NORMAL_USER_MODEL_DAILY_LIMIT_STATE_FILE',
    path.join(DATA_DIR, 'normal-user-model-daily-quota.json')
  ),
  ...
  GROUP_SUMMARY_STYLE: pick('GROUP_SUMMARY_STYLE', 'daily'),
  ...
  NORMAL_FAST_REPLY_ENABLED: pickBool('NORMAL_FAST_REPLY_ENABLED', false),
  NORMAL_FAST_REPLY_API_BASE_URL: pick('NORMAL_FAST_REPLY_API_BASE_URL', ''),
  NORMAL_FAST_REPLY_API_PROVIDER: pick('NORMAL_FAST_REPLY_API_PROVIDER', ''),
  NORMAL_FAST_REPLY_API_KEY: pick('NORMAL_FAST_REPLY_API_KEY', ''),
  NORMAL_FAST_REPLY_MODEL: pick('NORMAL_FAST_REPLY_MODEL', ''),
  ...
  NORMAL_FAST_REPLY_MAX_TOKENS: Math.max(64, pickNum('NORMAL_FAST_REPLY_MAX_TOKENS', 1024)),
  NORMAL_FAST_REPLY_PERSONA_MODULE_MAX_ACTIVE: Math.max(0, pickNum('NORMAL_FAST_REPLY_PERSONA_MODULE_MAX_ACTIVE', 2)),
  NORMAL_FAST_REPLY_PERSONA_MODULE_MAX_TOKEN_COST: Math.max(1, pickNum('NORMAL_FAST_REPLY_PERSONA_MODULE_MAX_TOKEN_COST', 100)),
  NORMAL_FAST_REPLY_PERSONA_MODULE_TEXT_MAX_CHARS: Math.max(100, pickNum('NORMAL_FAST_REPLY_PERSONA_MODULE_TEXT_MAX_CHARS', 700)),
  NORMAL_FAST_REPLY_WORLDBOOK_ENABLED: pickBool('NORMAL_FAST_REPLY_WORLDBOOK_ENABLED', true),
  NORMAL_FAST_REPLY_WORLDBOOK_MAX_ACTIVE: Math.max(0, pickNum('NORMAL_FAST_REPLY_WORLDBOOK_MAX_ACTIVE', 1)),
  NORMAL_FAST_REPLY_WORLDBOOK_MAX_TOKEN_COST: Math.max(1, pickNum('NORMAL_FAST_REPLY_WORLDBOOK_MAX_TOKEN_COST', 180)),
  NORMAL_FAST_REPLY_WORLDBOOK_TEXT_MAX_CHARS: Math.max(100, pickNum('NORMAL_FAST_REPLY_WORLDBOOK_TEXT_MAX_CHARS', 900)),
  NORMAL_GROUP_MAIN_REPLY_RPM_LIMIT_ENABLED: pickBool('NORMAL_GROUP_MAIN_REPLY_RPM_LIMIT_ENABLED', true),
  NORMAL_GROUP_MAIN_REPLY_RPM_LIMIT: Math.max(0, pickNum('NORMAL_GROUP_MAIN_REPLY_RPM_LIMIT', 12)),
  NORMAL_GROUP_MAIN_REPLY_RPM_WINDOW_MS: Math.max(0, pickNum('NORMAL_GROUP_MAIN_REPLY_RPM_WINDOW_MS', 60 * 1000)),
  ...
  RESEARCH_SUBAGENT_CACHE_MAX_SESSIONS: Math.max(1, pickNum('RESEARCH_SUBAGENT_CACHE_MAX_SESSIONS', 1000)),
  RESEARCH_SUBAGENT_CACHE_SWEEP_INTERVAL_MS: Math.max(1000, pickNum('RESEARCH_SUBAGENT_CACHE_SWEEP_INTERVAL_MS', 60 * 1000)),
  // ===== Tools =====
  ...
  PASSIVE_AWARENESS_VISUAL_CUE_PROBE_TIMEOUT_MS: pickNum('PASSIVE_AWARENESS_VISUAL_CUE_PROBE_TIMEOUT_MS', 3000),
  PASSIVE_AWARENESS_VISUAL_CUE_PROBE_RETRIES: pickNum('PASSIVE_AWARENESS_VISUAL_CUE_PROBE_RETRIES', 0),
  PASSIVE_AWARENESS_REPLY_API_BASE_URL: pick('PASSIVE_AWARENESS_REPLY_API_BASE_URL', pick('PASSIVE_AWARENESS_API_BASE_URL', '')),
  PASSIVE_AWARENESS_REPLY_API_PROVIDER: pick('PASSIVE_AWARENESS_REPLY_API_PROVIDER', ''),
  ...
  PASSIVE_AWARENESS_REPLY_USE_MAIN_MODEL: pickBool('PASSIVE_AWARENESS_REPLY_USE_MAIN_MODEL', false),
  ...
  CONTINUOUS_MESSAGE_GROUP_PLAIN_TEXT_DEBOUNCE_MS: pickNum('CONTINUOUS_MESSAGE_GROUP_PLAIN_TEXT_DEBOUNCE_MS', 2000),
  ...
  QZONE_HUMANIZE_PUBLISH_DELAY_ENABLED: pickBool('QZONE_HUMANIZE_PUBLISH_DELAY_ENABLED', true),
  QZONE_HUMANIZE_PUBLISH_DELAY_MIN_MS: Math.max(0, pickNum('QZONE_HUMANIZE_PUBLISH_DELAY_MIN_MS', 8000)),
  QZONE_HUMANIZE_PUBLISH_DELAY_MAX_MS: Math.max(0, pickNum('QZONE_HUMANIZE_PUBLISH_DELAY_MAX_MS', 35000)),
  ...
  MEME_MANAGER_FOLLOWUP_ENABLED: pickBool('MEME_MANAGER_FOLLOWUP_ENABLED', false),
  ...
  CONVERSATION_VARIABLES_ENABLED: pickBool('CONVERSATION_VARIABLES_ENABLED', true),
  CONVERSATION_VARIABLES_PRIMARY_READ: pickBool('CONVERSATION_VARIABLES_PRIMARY_READ', true),
  CONVERSATION_VARIABLES_MODEL_MIN_CONFIDENCE: pickNum('CONVERSATION_VARIABLES_MODEL_MIN_CONFIDENCE', 0.72),
  ...
  TOOL_AUTHORIZATION_TTL_MS: Math.max(1000, pickNum('TOOL_AUTHORIZATION_TTL_MS', 10 * 60 * 1000)),
  TOOL_AUTHORIZATION_DB_FILE: pick(
    'TOOL_AUTHORIZATION_DB_FILE',
    path.join(DATA_DIR, 'tool_authorizations.sqlite')
  ),
  ...
  SHORT_TERM_MEMORY_MAX_TOKENS: pickNum('SHORT_TERM_MEMORY_MAX_TOKENS', 9200),
  ADMIN_SHORT_TERM_MEMORY_MAX_TOKENS: pickNum(
    'ADMIN_SHORT_TERM_MEMORY_MAX_TOKENS',
    pickNum('HIGH_AFFINITY_SHORT_TERM_MEMORY_MAX_TOKENS', 9200)
  ),
  HIGH_AFFINITY_SHORT_TERM_MEMORY_MAX_TOKENS: pickNum(
    'HIGH_AFFINITY_SHORT_TERM_MEMORY_MAX_TOKENS',
    pickNum('ADMIN_SHORT_TERM_MEMORY_MAX_TOKENS', 9200)
  ),
  SHORT_TERM_MEMORY_COMPRESSION_TRIGGER_RATIO: pickNum('SHORT_TERM_MEMORY_COMPRESSION_TRIGGER_RATIO', 0.7),
  SHORT_TERM_MEMORY_SUMMARY_MAX_TOKENS: pickNum('SHORT_TERM_MEMORY_SUMMARY_MAX_TOKENS', 800),
  SHORT_TERM_MEMORY_RECENT_MESSAGES: pickNum('SHORT_TERM_MEMORY_RECENT_MESSAGES', 240),
  SHORT_TERM_MEMORY_RECENT_TURNS: pickNum('SHORT_TERM_MEMORY_RECENT_TURNS', 48),
  SHORT_TERM_SCENE_RECENT_TURNS: pickNum('SHORT_TERM_SCENE_RECENT_TURNS', 24),
  ...
  ...buildSessionContextRuntimeConfig({ pickNum }),
  ...
  SHORT_TERM_BRIDGE_RAW_TTL_HOURS: pickNum('SHORT_TERM_BRIDGE_RAW_TTL_HOURS', 48),
  SHORT_TERM_BRIDGE_RECENT_MESSAGES: pickNum('SHORT_TERM_BRIDGE_RECENT_MESSAGES', 96),
  ...
  MAIN_PROMPT_MEMORY_CONTEXT_MAX_TOKENS: pickNum('MAIN_PROMPT_MEMORY_CONTEXT_MAX_TOKENS', 2500),
  ...
  ...buildMainReplyContextRuntimeConfig({ pickNum }),
  ...
  MEMORY_RECALL_FORCE_LOCAL_RAG: pickBool('MEMORY_RECALL_FORCE_LOCAL_RAG', true),
  ...
  MEMORY_BM25_ENABLED: pickBool('MEMORY_BM25_ENABLED', true),
  MEMORY_RRF_ENABLED: pickBool('MEMORY_RRF_ENABLED', true),
  MEMORY_HYDE_ENABLED: pickBool('MEMORY_HYDE_ENABLED', false),
  MEMORY_MULTI_QUERY_ENABLED: pickBool('MEMORY_MULTI_QUERY_ENABLED', false),
  MEMORY_LOCAL_CANDIDATE_LIMIT: pickLiteBudgetNum('MEMORY_LOCAL_CANDIDATE_LIMIT', 96, 48),
  ...
  MEMORY_EMBEDDING_MODEL_VERSION: pick('MEMORY_EMBEDDING_MODEL_VERSION', ''),
  ...
  MEMORY_EMBEDDING_BACKFILL_BATCH_SIZE: pickLiteBudgetNum('MEMORY_EMBEDDING_BACKFILL_BATCH_SIZE', 32, 8),
  MEMORY_EMBEDDING_BACKFILL_MAX_PER_RUN: pickLiteBudgetNum('MEMORY_EMBEDDING_BACKFILL_MAX_PER_RUN', 128, 32),
  ...
  MEMORY_EMBEDDING_ENDPOINT_COOLDOWN_MS: pickNum('MEMORY_EMBEDDING_ENDPOINT_COOLDOWN_MS', pickNum('MEMORY_EMBEDDING_RETRY_COOLDOWN_MS', 1800000)),
  MEMORY_EMBEDDING_TRANSIENT_COOLDOWN_MS: pickNum('MEMORY_EMBEDDING_TRANSIENT_COOLDOWN_MS', 60000),
  MEMORY_EMBEDDING_FAILURE_THRESHOLD: Math.max(1, pickNum('MEMORY_EMBEDDING_FAILURE_THRESHOLD', 2)),
  ...
  MEMORY_QUERY_EMBEDDING_CACHE_TTL_MS: pickNum('MEMORY_QUERY_EMBEDDING_CACHE_TTL_MS', 10 * 60 * 1000),
  ...
  MEMORY_LANCEDB_READ_ENABLED: lowResourceDisableLanceDbHotPath
    ? false
    : pickBool('MEMORY_LANCEDB_READ_ENABLED', false),
  MEMORY_LANCEDB_SYNC_ENABLED: lowResourceDisableLanceDbHotPath
    ? false
    : pickBool('MEMORY_LANCEDB_SYNC_ENABLED', true),
  MEMORY_LANCEDB_TIMEOUT_MS: pickLiteBudgetNum('MEMORY_LANCEDB_TIMEOUT_MS', 800, 350),
  ...
  MEMORY_LANCEDB_PARTITION_MODE: pick('MEMORY_LANCEDB_PARTITION_MODE', 'legacy').toLowerCase(),
  MEMORY_LANCEDB_BUCKET_COUNT: Math.max(1, Math.min(256, pickNum('MEMORY_LANCEDB_BUCKET_COUNT', 32))),
  MEMORY_LANCEDB_LEGACY_FALLBACK_ENABLED: pickBool('MEMORY_LANCEDB_LEGACY_FALLBACK_ENABLED', true),
  MEMORY_LANCEDB_CANDIDATE_LIMIT: pickLiteBudgetNum('MEMORY_LANCEDB_CANDIDATE_LIMIT', 64, 16),
  MEMORY_LANCEDB_SYNC_BATCH_SIZE: pickLiteBudgetNum('MEMORY_LANCEDB_SYNC_BATCH_SIZE', 256, 64),
  MEMORY_LANCEDB_VECTOR_INDEX_TYPE: pick('MEMORY_LANCEDB_VECTOR_INDEX_TYPE', 'ivf_pq').toLowerCase(),
  MEMORY_LANCEDB_VECTOR_INDEX_NUM_BITS: Math.max(4, Math.min(8, pickNum('MEMORY_LANCEDB_VECTOR_INDEX_NUM_BITS', 8))),
  MEMORY_LANCEDB_VECTOR_INDEX_NUM_PARTITIONS: Math.max(0, pickNum('MEMORY_LANCEDB_VECTOR_INDEX_NUM_PARTITIONS', 0)),
  MEMORY_LANCEDB_VECTOR_INDEX_NUM_SUB_VECTORS: Math.max(0, pickNum('MEMORY_LANCEDB_VECTOR_INDEX_NUM_SUB_VECTORS', 64)),
  MEMORY_LANCEDB_VECTOR_INDEX_MIN_ROWS: Math.max(1, pickNum('MEMORY_LANCEDB_VECTOR_INDEX_MIN_ROWS', 256)),
  MEMORY_LANCEDB_VECTOR_INDEX_WAIT_TIMEOUT_SECONDS: Math.max(1, pickNum('MEMORY_LANCEDB_VECTOR_INDEX_WAIT_TIMEOUT_SECONDS', 120)),
  ...
  MEMORY_BM25_K1: pickNum('MEMORY_BM25_K1', 1.2),
  MEMORY_BM25_B: pickNum('MEMORY_BM25_B', 0.75),
  ...
  MEMORY_RERANK_MAX_CANDIDATES: pickLiteBudgetNum('MEMORY_RERANK_MAX_CANDIDATES', 40, 12),
  MEMORY_RERANK_SCORE_WEIGHT: pickNum('MEMORY_RERANK_SCORE_WEIGHT', 0.55),
  MEMORY_RERANK_TIMEOUT_MS: pickLiteBudgetNum('MEMORY_RERANK_TIMEOUT_MS', 2000, 800),
  MEMORY_RERANK_TIMEOUT_FLOOR_MS: Math.max(100, pickNum('MEMORY_RERANK_TIMEOUT_FLOOR_MS', 2000)),
  MEMORY_RERANK_TIMEOUT_COOLDOWN_MS: pickLiteBudgetNum('MEMORY_RERANK_TIMEOUT_COOLDOWN_MS', 60000, 30000),
  MEMORY_RERANK_TIMEOUT_FAILURE_THRESHOLD: Math.max(1, pickNum('MEMORY_RERANK_TIMEOUT_FAILURE_THRESHOLD', 2)),
  MEMORY_RERANK_ENDPOINT_COOLDOWN_MS: pickNum('MEMORY_RERANK_ENDPOINT_COOLDOWN_MS', 30 * 60 * 1000),
  MEMORY_RERANK_MAX_TIMEOUT_MS: pickLiteBudgetNum('MEMORY_RERANK_MAX_TIMEOUT_MS', 5000, 2000),
  MEMORY_RERANK_MAX_DOC_CHARS: pickLiteBudgetNum('MEMORY_RERANK_MAX_DOC_CHARS', 900, 420),
  MEMORY_RERANK_INSTRUCTION: pick('MEMORY_RERANK_INSTRUCTION', ''),
  MEMORY_RERANK_CANDIDATE_LIMIT: pickLiteBudgetNum('MEMORY_RERANK_CANDIDATE_LIMIT', 64, 8),
  ...
  MEMORY_JOURNAL_LONG_TERM_DEDUPE_ENABLED: pickBool('MEMORY_JOURNAL_LONG_TERM_DEDUPE_ENABLED', true),
  MEMORY_JOURNAL_LONG_TERM_DEDUPE_THRESHOLD: pickNum('MEMORY_JOURNAL_LONG_TERM_DEDUPE_THRESHOLD', 0.9),
  MEMORY_JOURNAL_LONG_TERM_DEDUPE_MAX_PAIRS: pickNum('MEMORY_JOURNAL_LONG_TERM_DEDUPE_MAX_PAIRS', 128),
  ...
  PERSONA_WORLDBOOK_EMBEDDING_ENABLED: lowResourceDisableWorldbookSemantic
    ? false
    : pickBool('PERSONA_WORLDBOOK_EMBEDDING_ENABLED', true),
  PERSONA_WORLDBOOK_EMBEDDING_HOT_PATH: pickBool('PERSONA_WORLDBOOK_EMBEDDING_HOT_PATH', false),
  PERSONA_WORLDBOOK_EMBEDDING_BACKFILL_MAX_PER_RUN: pickLiteBudgetNum('PERSONA_WORLDBOOK_EMBEDDING_BACKFILL_MAX_PER_RUN', 24, 6),
  PERSONA_WORLDBOOK_LEXICAL_LIMIT: pickLiteBudgetNum('PERSONA_WORLDBOOK_LEXICAL_LIMIT', 24, 8),
  PERSONA_WORLDBOOK_SEMANTIC_LIMIT: lowResourceDisableWorldbookSemantic
    ? 0
    : pickLiteBudgetNum('PERSONA_WORLDBOOK_SEMANTIC_LIMIT', 24, 6),
  PERSONA_WORLDBOOK_PLANNER_CANDIDATE_LIMIT: pickLiteBudgetNum('PERSONA_WORLDBOOK_PLANNER_CANDIDATE_LIMIT', 12, 6),
  PERSONA_WORLDBOOK_RERANK_ENABLED: lowResourceDisableWorldbookSemantic
    ? false
    : pickBool('PERSONA_WORLDBOOK_RERANK_ENABLED', true),
  PERSONA_WORLDBOOK_RERANK_MAX_CANDIDATES: pickLiteBudgetNum('PERSONA_WORLDBOOK_RERANK_MAX_CANDIDATES', 24, 8),
  PERSONA_WORLDBOOK_RERANK_TIMEOUT_MS: pickLiteBudgetNum('PERSONA_WORLDBOOK_RERANK_TIMEOUT_MS', 2000, 700),
  PERSONA_WORLDBOOK_SELECTED_MAX: pickNum('PERSONA_WORLDBOOK_SELECTED_MAX', 4),
  PERSONA_WORLDBOOK_SESSION_STATE_ENABLED: pickBool('PERSONA_WORLDBOOK_SESSION_STATE_ENABLED', true),
  ...
  PERSONA_WORLDBOOK_DB_FILE: pick('PERSONA_WORLDBOOK_DB_FILE', pick('PROFILE_JOURNAL_DB_FILE', path.join(DATA_DIR, 'profile_journal.sqlite'))),
  PERSONA_WORLDBOOK_DB_PRIMARY_READ: pickBool('PERSONA_WORLDBOOK_DB_PRIMARY_READ', true),
  ...
  MEMORY_V3_SESSION_RECENT_MESSAGES: pickNum('MEMORY_V3_SESSION_RECENT_MESSAGES', 128),
  ...
  MEMORY_PROFILE_IDENTITY_NOISE_FILTER: pickBool('MEMORY_PROFILE_IDENTITY_NOISE_FILTER', true),
  MEMORY_PROFILE_CURRENT_USER_ANCHOR: pickBool('MEMORY_PROFILE_CURRENT_USER_ANCHOR', true),
  ...
  PROFILE_JOURNAL_DB_ENABLED: pickBool('PROFILE_JOURNAL_DB_ENABLED', true),
  PROFILE_JOURNAL_DB_FILE: pick('PROFILE_JOURNAL_DB_FILE', path.join(DATA_DIR, 'profile_journal.sqlite')),
  PROFILE_JOURNAL_DB_PRIMARY_READ: pickBool('PROFILE_JOURNAL_DB_PRIMARY_READ', true),
  PROFILE_JOURNAL_AUTO_CLEAN_ENABLED: pickBool('PROFILE_JOURNAL_AUTO_CLEAN_ENABLED', true),
  PROFILE_JOURNAL_AUTO_CLEAN_INTERVAL_MS: Math.max(0, pickNum('PROFILE_JOURNAL_AUTO_CLEAN_INTERVAL_MS', 60000)),
  ...
  MEMORY_WRITE_REVIEW_TIMEOUT_FAILURE_THRESHOLD: pickNum('MEMORY_WRITE_REVIEW_TIMEOUT_FAILURE_THRESHOLD', 2),
  MEMORY_WRITE_REVIEW_TIMEOUT_COOLDOWN_MS: pickNum('MEMORY_WRITE_REVIEW_TIMEOUT_COOLDOWN_MS', 60000),
  ...
  MEMORY_EMBEDDING_BACKFILL_CONCURRENCY: Math.max(1, pickNum('MEMORY_EMBEDDING_BACKFILL_CONCURRENCY', 2)),
  ...
  MEMORY_JOURNAL_UNSAFE_REPLY_FILTER: pickBool('MEMORY_JOURNAL_UNSAFE_REPLY_FILTER', true),
  DAILY_JOURNAL_SUMMARY_SCHEDULER_ENABLED: pickBool('DAILY_JOURNAL_SUMMARY_SCHEDULER_ENABLED', true),
  DAILY_JOURNAL_SUMMARY_SCHEDULER_INTERVAL_MS: Math.max(60 * 1000, pickNum('DAILY_JOURNAL_SUMMARY_SCHEDULER_INTERVAL_MS', 10 * 60 * 1000)),
  DAILY_JOURNAL_TURN_COMPACTION_ENABLED: pickBool('DAILY_JOURNAL_TURN_COMPACTION_ENABLED', true),
  DAILY_JOURNAL_TURN_COMPACTION_THRESHOLD: Math.max(1, pickNum('DAILY_JOURNAL_TURN_COMPACTION_THRESHOLD', 50)),
  DAILY_JOURNAL_TURN_COMPACTION_MAX_INPUT_CHARS: Math.max(4000, pickNum('DAILY_JOURNAL_TURN_COMPACTION_MAX_INPUT_CHARS', 40000)),
  DAILY_JOURNAL_TURN_COMPACTION_RECALL_LIMIT: Math.max(1, pickNum('DAILY_JOURNAL_TURN_COMPACTION_RECALL_LIMIT', 8)),
  ...
  DAILY_JOURNAL_SEGMENT_MAX_ENTRIES: pickNum('DAILY_JOURNAL_SEGMENT_MAX_ENTRIES', 6),
  ...
  DAILY_JOURNAL_SEGMENT_MIN_PENDING_ENTRIES: Math.max(1, pickNum('DAILY_JOURNAL_SEGMENT_MIN_PENDING_ENTRIES', 10)),
  DAILY_JOURNAL_SEGMENT_MAX_PENDING_AGE_MS: Math.max(0, pickNum('DAILY_JOURNAL_SEGMENT_MAX_PENDING_AGE_MS', 30 * 60 * 1000)),
  DAILY_JOURNAL_SEGMENT_CLUSTER_ENABLED: pickBool('DAILY_JOURNAL_SEGMENT_CLUSTER_ENABLED', true),
  DAILY_JOURNAL_SEGMENT_MAX_CLUSTERS_PER_RUN: Math.max(1, pickNum('DAILY_JOURNAL_SEGMENT_MAX_CLUSTERS_PER_RUN', 3)),
  DAILY_JOURNAL_SEGMENT_MIN_CLUSTER_ENTRIES: Math.max(1, pickNum('DAILY_JOURNAL_SEGMENT_MIN_CLUSTER_ENTRIES', 2)),
  ...
  CONVERSATION_VARIABLES_DB_FILE: pick('CONVERSATION_VARIABLES_DB_FILE', path.join(DATA_DIR, 'conversation_variables.sqlite')),
  ...
  IMAGE_MEMORY_VISUAL_SUMMARY_CONCURRENCY: Math.max(1, pickNum('IMAGE_MEMORY_VISUAL_SUMMARY_CONCURRENCY', 2)),
