...
### 模型与 Provider
- **21:41** 启用普通用户主回复备用模型：`AI_FALLBACK_ENABLED=true`，备用模型为 `gemini-3.7-flash`，连续失败 3 次后切换，冷却 10 分钟；管理员备用模型保持关闭。

## 2026-08-25

### Prompt
- **11:11** 本地私有 `prompts/admin.txt` 补恋爱承接和成人向规则。真实管理员模型验收：恋爱回复无拒绝；R18 回复写到进入和高潮。文件仍被 gitignore，不入库。


## 2026-08-05

### Bug 修复
- **00:10** 修复测试进程误连真实 NapCat：测试运行器在未显式提供地址时使用隔离回环端口，并保留自定义 mock 地址。实现提交 `6acffdc`；4 项定向测试、目标 ESLint、typecheck 与 diff 检查通过，小目标已完成。
