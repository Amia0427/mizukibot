    ...
    appendInboundTimingLog(inboundTimingLogFile, config.ENABLE_DEBUG_LOG, {
      ...
      continuousWaitMs: Number(preprocessed?.meta?.timing?.waitMs || 0) || 0,
      continuousResolveMs: Number(preprocessed?.meta?.timing?.resolveDetailsMs || 0) || 0,
      continuousTotalMs: Number(preprocessed?.meta?.timing?.totalMs || 0) || 0,
      continuousSessionAgeMs: Number(preprocessed?.meta?.timing?.sessionAgeMs || 0) || 0,
      continuousEntryCount: Number(preprocessed?.meta?.timing?.entryCount || 0) || 0,
      continuousTimerOverdueMs: Number(preprocessed?.meta?.timing?.timerOverdueMs || 0) || 0,
      continuousTimerActualDelayMs: Number(preprocessed?.meta?.timing?.timerActualDelayMs || 0) || 0,
      continuousScheduleDelayMs: Number(preprocessed?.meta?.timing?.scheduleDelayMs || 0) || 0,
      continuousScheduleDebounceMs: Number(preprocessed?.meta?.timing?.scheduleDebounceMs || 0) || 0,
      continuousScheduleMaxHoldMs: Number(preprocessed?.meta?.timing?.scheduleMaxHoldMs || 0) || 0,
