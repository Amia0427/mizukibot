...
const innerThoughtSchema = z.object({
  inner_thought: z.string().trim().min(1).max(40)
}).strict();
...
function buildStatusBarInstructions() {
  return [
    ...
    '只输出一个 JSON 对象，且只能有 inner_thought 字段；inner_thought 使用瑞希第一人称，简体中文，1-40 字。'
  ].join('\n');
}
...
function createPrivateStatusBarModelClient(runtimeConfig = {}, options = {}) {
  ...
  return async function requestInnerThought(input = {}) {
    ...
    try {
      ...
      const validated = innerThoughtSchema.safeParse(parsed.value);
