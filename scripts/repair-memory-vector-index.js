...
function parseArgs(argv = process.argv.slice(2)) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (item === '--dry-run') {
      ...
      args.dir = item.slice('--dir='.length).trim();
    } else if (item === '--partition-mode') {
      args.partitionMode = String(argv[index + 1] || '').trim();
      index += 1;
    } else if (item.startsWith('--partition-mode=')) {
      args.partitionMode = item.slice('--partition-mode='.length).trim();
    } else if (item === '--bucket-count') {
      args.bucketCount = Math.max(1, Number(argv[index + 1] || 0) || 0);
      index += 1;
    } else if (item.startsWith('--bucket-count=')) {
      args.bucketCount = Math.max(1, Number(item.slice('--bucket-count='.length)) || 0);
    } else if (item === '--source') {
...
async function runRepair(args = {}, deps = {}) {
  const options = {
    ...
    partitionMode: String(args.partitionMode || '').trim(),
    bucketCount: Math.max(0, Number(args.bucketCount || 0) || 0)
  };
  const lanceDbOptions = buildLanceDbOptions(options);
