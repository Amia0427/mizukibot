...
- `..\restart-bot.cmd`：Windows 手动/远程重启入口；更新 2026-06-17 01:05 +08:00，无参数默认只读 status，显式 `restart` 才会停启主 bot；写 `data/bot-main-expected-shutdown.json` 前会确认目标 PID 是存活的 `node index.js`，避免 stale lock 被误标为 expected shutdown
