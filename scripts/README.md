...
- `restart-bot-periodic.ps1`：Windows 定时重启脚本；更新 2026-08-18 10:35 +08:00，计划任务每天 04:00 运行，直接启动 `node.exe index.js` 并将 stdout/stderr 重定向到 `data\bot-runtime.out.log` 与 `data\bot-runtime.err.log`；更新 2026-06-10 23:51 +08:00，计划任务默认每天 04:00 运行；更新 2026-06-08 13:36 +08:00，直接解析 `node.exe` 启动 `index.js`，避免计划任务 `Start-Process npm` 命中 shim 报 `%1 不是有效的 Win32 应用程序`。验收：`node tests\periodicRestartScript.test.js`、PowerShell AST parse 通过；2026-08-18 10:35 直接执行周期脚本后旧 PID `44816` 更新为 `33892`，`.mizukibot.lock` 持有且 bot 运行，runtime stdout/stderr mtime 更新并分别为 435/393 字节。
...
- `diagnose-main-model-retry-duplicates.js`：更新 2026-06-24 09:40 +08:00，只读交叉扫描 `request-trace.ndjson` 和 `model-calls.ndjson`，识别同一 requestId 在 HTTP 408 retryable 后继续主模型调用的疑似重复样本；可用 `npm run diag:main-model-retry-duplicates -- --around "2026-06-24T00:47:59+08:00" --window 5m --admin-only`
...
- `diagnose-memory-rag-explain.js`：更新 2026-06-24 18:01 +08:00，只读输出一次主回复本地 Memory V3 RAG explain/diagnostic，直接按真实 `userId + query` 展示候选来源、journal segment 命中、long-term/profile 命中、rank fusion / rerank、journal-vs-long-term 去重，以及最终保留结果；支持 `--user-id`、`--query/--text`、`--session-key`、`--group-id`、`--facet`、`--source`、`--top-k`、`--stage-limit`、`--max-chars`、`--data-dir`。验收：`node tests/memoryV3RagExplainDiagnostic.test.js`、`node tests/memoryV3RagExplainDedupStage.test.js`。
