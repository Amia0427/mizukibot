...
- `restart-bot-periodic.ps1`：Windows 定时重启脚本；更新 2026-08-27 09:42 +08:00，计划任务默认每天 09:30、21:30 运行，本机 `MizukiBotPeriodicRestart` 已重新注册并确认两个触发器，下一次运行时间为 2026-08-27 21:30；更新 2026-08-18 10:35 +08:00，直接启动 `node.exe index.js` 并将 stdout/stderr 重定向到 `data\bot-runtime.out.log` 与 `data\bot-runtime.err.log`；更新 2026-06-10 23:51 +08:00，计划任务默认每天 04:00 运行；更新 2026-06-08 13:36 +08:00，直接解析 `node.exe` 启动 `index.js`，避免计划任务 `Start-Process npm` 命中 shim 报 `%1 不是有效的 Win32 应用程序`。验收：`node tests\periodicRestartScript.test.js`、PowerShell AST parse、计划任务触发器实查通过。
