    ...
    if (requestTrace) {
      for (const event of events) {
        ...
        appendRequestTraceEvent(nextTracePhase(requestTrace, `runtime_v2_${eventType}`, {
          ...
          stopReason: String(event?.reason || event?.stopReason || '').trim(),
          status: String(event?.status || '').trim(),
          blockedReason: String(event?.blockedReason || '').trim(),
          toolName: String(event?.toolName || '').trim(),
          toolCallId: String(event?.toolCallId || '').trim(),
          toolRoundCount: Number.isFinite(Number(event?.toolRoundCount)) ? Math.max(0, Math.floor(Number(event.toolRoundCount))) : undefined,
          toolCallCount: Number.isFinite(Number(event?.toolCallCount)) ? Math.max(0, Math.floor(Number(event.toolCallCount))) : undefined,
          round: Number.isFinite(Number(event?.round)) ? Math.max(0, Math.floor(Number(event.round))) : undefined,
          callCount: Number.isFinite(Number(event?.callCount)) ? Math.max(0, Math.floor(Number(event.callCount))) : undefined,
