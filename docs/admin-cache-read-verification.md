...
更新时间：2026-06-22 12:32 +08:00

## 2026-06-22 更新

- 12:32：复查主回复仍只写不读，确认最终请求同时存在 `system_cache_breakpoints=1` 和 `message_cache_breakpoints=1`；目标兼容网关疑似按最后 `cache_control` 断点建缓存，历史 messages 前缀会随对话窗口变化。现有稳定 system 断点时不再保留 messages 断点；无 system 断点时才缓存倒数第 2 条非空历史消息。真实双请求验收显示最终请求体一致，`system_cache_breakpoints=1`、`message_cache_breakpoints=0`、`ttl=5m`、`X-Enable-1h-cache=1`；该端点本次响应未返回 usage/cache 字段。
