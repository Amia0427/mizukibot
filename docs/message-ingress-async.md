...
更新 2026-08-28：第一阶段可靠性改造已取消消息处理链中的 freshness 发送门控。连续消息预处理仍可合并短时间内的输入，但消息一旦进入正式处理链路，不会因为同一会话后续消息到达而丢弃普通、快速或流式回复；同一 sessionKey 仍串行，不同会话仍按 active 并发上限并行。入口 dispatcher、入站控制器和前台控制器现在提供累计接受、启动、获取、释放、峰值排队和最长等待统计。
