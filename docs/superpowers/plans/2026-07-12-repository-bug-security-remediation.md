...
- [ ] 增加私网 DNS、混合地址和重定向到私网的失败测试。
- [ ] 请求前解析并校验全部地址，禁用 Axios 自动重定向并逐跳校验。
- [ ] 复用统一安全请求边界，避免两套判断漂移。
- [ ] 验证公网请求兼容性与所有绕过测试。
- [ ] 更新维护日志并提交。
...
- [ ] 增加已产生副作用但响应超时的重复发送测试。
- [ ] 仅对明确未送达且可安全重试的错误重试；保留错误分类。
- [ ] 验证逻辑消息最多发送一次。
- [ ] 更新维护日志并提交。
