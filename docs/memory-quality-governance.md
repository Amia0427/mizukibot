...
更新 2026-06-16 08:35 +08:00：`memoryReranker` 连续超时治理。6/16 凌晨 `data/bot-runtime.err.log` 的 rerank timeout 与 `normal_fast_reply` 回 formal 同窗口出现；按 `data/request-trace.ndjson` 复核，`req_f91837e5868378ff` 的正式回落直接原因是 fast reply HTTP 200 后本地 `generic_model_failure`，reranker 超时是记忆召回降级/延迟放大因素。`data/model-calls.ndjson` 同时显示 rerank hard-timeout 后底层请求仍可能最终以 43577ms 成功落盘，导致并发查询继续击穿慢端点。现 `utils/memoryReranker.js` 默认只允许同进程一个 rerank in-flight；忙碌时后续查询直接使用 base recall，并在 runtime state 暴露 `inFlight/skippedInFlight`。验收：`node tests\memoryReranker.test.js`、`node tests\memoryHybridRerankPipeline.test.js`、`node tests\memoryV3Bm25RrfDiagnostics.test.js`、`node tests\hotpathRequireGuard.test.js`、`npm run diag:route-decision -- --request-id req_f91837e5868378ff --limit 1`、`npm run diag:runtime-exceptions -- --since 2026-06-16T00:00:00+08:00 --limit 20` 通过。小目标完成：慢 rerank 端点不再被并发连续超时放大。
