...
Windows daemon、`scripts/one-click-start.ps1` 和 Linux fallback 启动都会先扫描现有 worker 进程；PID 文件缺失但进程存在时只补 PID，不会误杀正常任务，也不会再起第二个 worker。Windows daemon 在 worker 不存在时会检查队列：存在 queued job 或租约已过期的 processing job 会补启；队列空闲且主 bot 原本已在运行时，只有显式开启 `POST_REPLY_WORKER_IDLE_RECYCLE_ENABLED=true` 才记录 `queue idle; skip idle restart`；默认常驻模式会补启缺席 worker。如果本轮 daemon 刚成功拉起主 bot，则仍会在外置 worker 启用时补启一次并写更具体原因。默认情况下外置 worker 不再执行 RSS idle recycle；若显式开启 `POST_REPLY_WORKER_IDLE_RECYCLE_ENABLED=true`，worker 仍会在 queued/processing 未清空时跳过 recycle，`processing` job 靠 `leaseOwner/leaseUntil` 做恢复边界，只有租约过期才会由下一轮 worker recovery。
...
| `POST_REPLY_WORKER_CONCURRENCY` | `2` | worker 并发上限 |
| `POST_REPLY_WORKER_PRESSURE_MAX_CONCURRENCY` | `1` | 资源压力态 worker 有效并发上限 |
| `POST_REPLY_WORKER_STALE_PROCESSING_MS` | `300000` | processing 租约/旧 job 恢复窗口 |
| `POST_REPLY_WORKER_IDLE_RECYCLE_ENABLED` | `false` | 是否允许外置 worker 空闲时按 RSS 阈值主动退出；默认常驻 |
