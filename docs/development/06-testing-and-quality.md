...
| 全部生产代码 | 69% | 78% | 61% |
| `web/` | 78% | 83% | 79% |
| Runtime V2 非 chunk | 74% | 63% | 58% |
| 稳定安全边界 | 84% | 80% | 70% |

Function 覆盖率依赖 V8 的函数区间统计，基线以项目唯一运行边界 Node 20.x 生成；不要用其他 Node 主版本的结果更新 Function 阈值。不要通过排除新文件、下调已校准基线或只测 getter 来“修复”覆盖率，新增公共分支应由行为断言覆盖。
