...
本文面向需要修改消息入口、路由、Agent 图、工具执行、回复发送或后台副作用的开发者。它描述当前分支真实运行链路，而不是目录名暗示的理想架构。最后核验：2026-08-11 22:17 +08:00。
...
普通发送分支和完成的流式发送分支都调用同一个 freshness guard；新消息到达后，旧回合在独立模型、渲染和发送前都会被丢弃。路由计划、工具授权、拒绝/限流/故障标记由状态栏运行时再次检查，因此不要在群聊、命令或工具分支手动调用它。

状态栏独立模型只接受严格 `{"inner_thought":"..."}`，不接收工具；固定模板负责所有标签、CSS、属性和尺寸，动态值只进入实体转义后的文本节点。模型失败、输出守卫、敏感词审查、渲染或 QQ 发送失败均静默降级，不应添加用户可见兜底。
