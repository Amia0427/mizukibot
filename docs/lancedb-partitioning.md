...
- 旧 `data/lancedb` 只作为回滚保留，不在迁移脚本中删除。
...
MEMORY_LANCEDB_LEGACY_FALLBACK_ENABLED=true
```

`MEMORY_LANCEDB_PARTITION_MODE=legacy` 会保持旧单表行为。`MEMORY_LANCEDB_LEGACY_FALLBACK_ENABLED=true` 只影响 bucket 读路径：目标 bucket 未命中时允许回查旧单表，验证稳定后可改为 `false`。
...
验证通过后更新 `.env` 并重启主进程与 post-reply worker。回滚只需要把 `.env` 改回 `MEMORY_LANCEDB_DIR=./data/lancedb` 和 `MEMORY_LANCEDB_PARTITION_MODE=legacy` 后重启。
