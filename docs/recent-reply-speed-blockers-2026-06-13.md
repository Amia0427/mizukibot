...
- 2026-06-17 08:57 +08:00 更新：复核 `req_de9d93fa46f70729`、`req_24de5257c938f3ae`、`req_56e9e0f52cc3b40a`、`req_426e8a3e3362efcf`、`req_85e0879302b5f613`，5 条 `message_ingress_lock_acquired.queueWaitMs=0-1ms`、`inbound_wait_ms=0`，不是 `general` lane 排队或释放时机问题；15s-26s 来自入锁前连续消息窗口，当前 `.env` 的 `CONTINUOUS_MESSAGE_DEBOUNCE_MS=15000` 和 `MAX_HOLD=25000` 正好对应现场。已新增 `CONTINUOUS_MESSAGE_GROUP_PLAIN_TEXT_DEBOUNCE_MS` 默认 2000ms，普通群非 @bot 且无图片/转发/卡片锚点时用短等待；图片/转发/卡片、@bot 和私聊仍走原窗口。验收：当前配置探针 `regular=2000, anchored=15000, atBot=12000, private=12000`；`node tests/continuousMessagePreprocessor.test.js`、`node tests/continuousMessagePreprocessorDebounce.test.js`、`node tests/messageHandlerGroupConcurrency.test.js`、`node tests/messageHandlerInboundConcurrency.test.js`、`node -e "require('./core/messageHandler'); console.log('message handler load ok')"` 通过。
