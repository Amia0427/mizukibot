...
function buildLanceDbReadMigrationGate(input = {}, options = {}) {
  ...
  const minReady = Math.max(0, Math.min(1, Number(options.minReadyRatio ?? 0.95) || 0.95));
  const failures = [];
  ...
  if (minReadyRatio(coverage) < minReady) failures.push('embedding_coverage_below_threshold');
  ...
  return {
    ...
    metrics: {
      minReadyRatio: minReadyRatio(coverage),
