...
const { shouldLockPassiveReply } = require('../../../utils/socialContextRuntime');
...
function classifyPassiveReplyType({ text, addressee, analysis }) {
  ...
  if (/(坏掉|坏了|变笨|变蠢|抽风|失忆|卡住|死机|智障|傻了|没电)/i.test(t)) return 'light_tease';
...
module.exports = {
  ...
  buildPassiveModelUserContent,
  buildPassiveVisualPromptSection,
  buildPresenceReason,
  canCallDecisionModel,
  canCallReplyModel,
  classifyPassiveReplyType,
  collectPassiveVisualInputs,
  containsBotPresenceCue,
  containsBotTopic,
