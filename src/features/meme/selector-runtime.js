...
const {
  extractSelectorResponseText,
  normalizeSelectorResult,
  parseLooseSelectorOutput,
  previewSelectorText,
  uniqueStrings
} = require('./context');
const {
  chooseCategoryBySelector,
  getIntensityDistance,
  inferCategoryByLocalHeuristics
} = require('./gate');
const runtimeState = require('./runtime-state');
const { buildRuntimeSummary, getFollowupRuntime } = runtimeState;
const { resolveAssetAnalysis } = require('./asset-analysis-runtime');
...
function getAssetGlobalUsage(assetId = '') {
  ...
  const state = runtimeState.runtimeStoreCache.assets[key];
  return {
...
module.exports = {
  buildSelectorPrompt,
  tokenizeOverlapTerms,
  getReplyContextTags,
  getAssetGlobalUsage,
  scoreAssetKeywordOverlap,
  compareAssetScores,
