...
const { resolveMainReplyPromptMode } = require('../../../utils/mainReplyPromptMode');
...
function normalizePlannerDynamicContextPlan(options = {}) {
  ...
  if (plannerPlan) {
    ...
    return ensureGroupDirectPersonaModulePlan({
...
function getPromptBlockPlanIds(block = {}) {
  ...
  return { blockId, aliasId, moduleId, ids: [blockId, aliasId].filter(Boolean) };
}
...
function blockHasUsableContent(block = {}) {
  ...
  const pattern = emptyPatternByBlock[aliasId || blockId];
...
function filterBlocksByPlan(blocks = [], dynamicPromptPlan = {}, options = {}) {
  ...
  const candidateBlocks = blockedIds.size > 0
    ? normalizeArray(blocks).filter((block) => !getPromptBlockPlanIds(block).ids.some((id) => blockedIds.has(id)))
    : blocks;
  ...
  if (audit) {
    for (const entry of normalizeArray(selection.runtimeAddedBlocks)) pushUniqueAuditEntry(audit.runtimeAddedBlocks, entry);
    for (const entry of normalizeArray(selection.runtimeRejectedBlocks)) pushUniqueAuditEntry(audit.runtimeRejectedBlocks, entry);
...
function ensureGroupDirectPersonaModulePlan(plan = {}, options = {}) {
  ...
  if (topRouteType !== 'direct_chat' || !groupId) return cloned;
...
module.exports = {
  ...
  filterBlocksByPlan,
  findPlannerDynamicPromptPlan,
  getPromptBlockPlanIds,
  normalizeDynamicPromptPlan,
  normalizePlannerBlockDecisions,
  normalizePlannerDynamicContextPlan,
  planHasBlockDecision,
  planIncludesBlock,
  planSkipsBlock,
