...
const { buildGeminiRecentStyleGuardPrompt } = require('../../../utils/geminiRecentStyleGuard');
const { buildBaseDynamicPrompt } = require('./base');
const { renderPromptLayers } = require('./render-runtime');
const {
  cloneDynamicPromptPlan,
  createDynamicContextAudit,
  filterBlocksByPlan,
  getPromptBlockPlanIds,
  normalizeDynamicPromptPlan,
  pushUniqueAuditEntry,
  shouldBlockAmbientMemoryForPlainChat,
  shouldRuntimeAddRetrievedMemoryBlock
} = require('./plan');
const {
...
const { shouldExposeMemoryCli } = require('./memory-inputs-core');
const {
  buildCacheFriendlyFingerprint,
  buildPromptBlockFingerprint,
  buildPromptCacheKeys,
  buildSessionCacheFingerprint,
  clonePromptBlocks,
  clonePromptLayerValue,
  dedupePromptBlocks,
  excludePromptBlocks,
  extractSessionStablePromptBlocks,
  promptLayerCache,
  prunePromptLayerCache,
  splitBlocksByLane,
  withSoftTimeout
} = require('./cache-runtime');

const { collectPromptInputs } = require('./prompt-inputs');
const {
  buildContinuityStatePromptSnippet,
...
const {
  ...
  buildMemoryRecallPolicyPromptSnippet,
  dedupeMemosRecallForPrompt,
  dedupeOpenVikingRecallForPrompt,
  normalizeOpenVikingRecallBlockText,
  resolveMemoryPromptBudgetMs,
  resolveMemosRecallObject,
  resolveMemosRecallText,
  resolveOpenVikingRecallObject,
...
const { blocksToMessages, createLiveStatePromptBlock, createPromptBlock, serializePromptBlocks } = require('./prompt-blocks-runtime');
const {
...
const { hashText, normalizeArray, normalizeText } = require('./normalization');

function buildSharedShortTermContextMessages(...args) {
...
function buildPersonaModuleCandidates(...args) {
  return require('../../../utils/personaModules').buildPersonaModuleCandidates(...args);
}

function loadPersonaModuleText(...args) {
  return require('../../../utils/personaModules').loadPersonaModuleText(...args);
}
...
async function buildDynamicPrompt(userInfo, userId, question, customPrompt = null, options = {}) {
  ...
  if (liveStateContextText) {
    runtimeAddedIds.push('live_state_dynamic');
  }
  if (geminiRecentStyleGuardText) {
