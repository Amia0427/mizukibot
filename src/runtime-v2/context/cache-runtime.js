...
const { getAffinitySettings } = require('../../../utils/contextBudget');
const { resolveChatSurface } = require('../../../utils/chatLivenessContext');
const { DYNAMIC_CONTEXT_PLAN_VERSION } = require('./config');
...
function clonePromptPlanForCache(plan = {}) {
...
function clonePromptLayerValue(value = {}) {
  ...
  const promptSnapshot = normalized.promptSnapshot && typeof normalized.promptSnapshot === 'object'
    ? {
        ...
        plannerDynamicContextPlan: clonePromptPlanForCache(normalized.promptSnapshot.plannerDynamicContextPlan),
  ...
  return {
    ...
    dynamicPromptPlan: normalized.dynamicPromptPlan && typeof normalized.dynamicPromptPlan === 'object'
      ? clonePromptPlanForCache(normalized.dynamicPromptPlan)
      : {},
...
function buildPromptSurface(topRouteType = '', routeMeta = {}) {
  ...
  return resolveChatSurface({ topRouteType, routeMeta: normalizedRouteMeta });
}
