...
module.exports = queryMemory({
  ...
  assert.ok(['semantic', 'hybrid', 'hybrid_rrf'].includes(result.results[0].matchMode));
  assert.ok(Number(result.results[0].embedding || result.results[0].scoreParts?.embedding || 0) > 0);
