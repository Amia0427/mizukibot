...
module.exports = (async () => {
  ...
  const previousWritePipeline = require('../config').MEMORY_WRITE_PIPELINE_ENABLED;
  const previousRerankEnabled = require('../config').MEMORY_RERANK_ENABLED;
  require('../config').MEMORY_WRITE_PIPELINE_ENABLED = true;
  require('../config').MEMORY_RERANK_ENABLED = true;
  try {
    const lowConfidenceWrite = await addMemoryItemsBatchWithVectorBackfill([{
      userId: 'u_pipe_guard',
      type: 'fact',
      text: 'unsafe low confidence rerank must not revive this memory',
      source: 'test',
      ...
      materialize: false,
      requestRerank: async () => [{ index: 0, score: 1 }, { index: 1, score: 0 }]
    });
    assert.strictEqual(lowConfidenceWrite.ids.length, 0, 'rerank should not restore pipeline-rejected low confidence memory');
    assert.ok(lowConfidenceWrite.rejected.some((item) => item.reason === 'low_confidence'), 'low confidence rejection should be preserved');
  } finally {
    require('../config').MEMORY_WRITE_PIPELINE_ENABLED = previousWritePipeline;
    require('../config').MEMORY_RERANK_ENABLED = previousRerankEnabled;
  }
