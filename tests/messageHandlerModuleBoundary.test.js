...
assert.strictEqual(legacyBindings.variables.length, 161, 'legacy top-level binding count');
assert.strictEqual(new Set(legacyBindings.functions).size, 46, 'legacy function names must be unique');
assert.strictEqual(new Set(legacyBindings.variables).size, 161, 'legacy variable bindings must be unique');
assert.strictEqual(
