...
module.exports = (async () => {
  ...
  try {
    ...
    assert.strictEqual(geminiReport.requested.provider, 'openai_compatible');
    assert.strictEqual(geminiDirect.finalProvider, 'openai_compatible');
    assert.strictEqual(geminiDirect.auth.header, 'Authorization');
    ...
    assert.strictEqual(geminiDirect.cache.openaiPromptCacheKey, 'provider-diagnostic-cache-key');
    assert.strictEqual(geminiDirect.cache.openaiPromptCacheRetention, '24h');
    assert.ok(findRemoved(geminiDirect.strippedFields, 'cache_control') > 0);
    assert.strictEqual(geminiDirect.geminiSystemInstruction, null);
    assert.ok(geminiDirect.anomalies.includes('openai_prompt_cache_and_cache_control_both_present'));
