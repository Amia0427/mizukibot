...
- 保留 `image_qa` / `image_summary` 路由、多图输入、视觉上下文预算和独立 caption worker，不影响图片识别与总结能力。
- 验收：4 项视觉定向测试、4 项提示词/加载定向测试中的 3 项、lint（911 个文件）、typecheck 和差异检查通过；`check:prompts` 及其集成测试仅被仓库既有的 `ADULT.txt` 未纳入 manifest 问题阻断，本次删除资产已通过清单检查。实现提交：`260494a8`。

## 陪伴功能更新公告 2026-09-01 08:45 +08:00

- 新增面向用户的[陪伴功能更新公告](docs/companion-update-announcement-2026-09-01.md)，包含待跟进事项、陪伴回顾、用户可控记忆、共读共看共听、QQ 私聊按需语音和 PWA 陪伴房间的短版群公告与长版更新日志。
- 公告已按当前实现标明私聊范围、默认关闭项、启用条件、媒体与离线边界；未将共读共看共听描述为真实媒体播放，也未将回顾描述为自动定时推送。
...
- 文件仍被 `.gitignore` 忽略，不进入版本库；普通用户 `defaut.txt`、代码装配链路和模型配置未改。
- 验收（2026-08-25 11:11 +08:00）：管理员稳定块成功注入 `admin_system_prompt`。恋爱实测回复含“喜欢你啊”，无政策拒绝。R18 实测 519 字，含接吻、脱衣、进入和高潮，无政策拒绝、无软挡。模型为 `claude-opus-4-6-thinking`。未推送远端。


## QQ 私聊重启续传 2026-08-25 01:04 +08:00

- HTTP 反向入口现在会在返回 `204` 前同步保存私聊入站消息；只有正常消息管线完整结束后才标记完成，进程异常退出时未完成消息会保留到下次启动。
- 启动后会查询 NapCat 最近会话与好友历史，补齐停机窗口内未投递到进程的最新私聊；恢复不重放斜杠命令，恢复前发现用户消息后已有机器人文字回复时只修复状态，不重复发送。
- 默认开启，状态文件为 `DATA_DIR/private-message-restart-recovery.json`；范围、配置、失败重试和验收记录见 [QQ 私聊重启续传](docs/private-message-restart-recovery.md)。
- 验收（2026-08-25 01:04 +08:00）：定向测试、NapCat 入站 smoke、重启回归、904 文件 lint、typecheck、密钥扫描和差异检查均通过；完整测试的既有 prompt 治理失败已记录在功能文档，未修改提示词。
...
## 私聊共读共看共听 2026-08-25 01:09 +08:00

- 现有 `/陪伴` 共处房间新增共读、共看、共听模式，可指定内容标题和 15/30/45/60/120 分钟时长，并用 `/陪伴 进度 <内容>` 记录当前进度。
- 开始、状态、节点消息和共同回忆会携带标题与最后进度；旧房间与旧回忆继续兼容。Bot 不下载或播放媒体，也不会声称自己实际读过、看过或听过内容。
- 验收（2026-08-25 01:09 +08:00）：companion-room 六项功能测试及两项 tick 相邻回归全部通过；`npm run lint` 检查 904 个文件，`npm run typecheck` 与 `git diff --check` 均退出 0。

## 私聊用户可控记忆 2026-08-25 11:15 +08:00

- 新增 `companion_memory` 私聊工具，支持查看、明确保存、更正和遗忘当前用户的长期记忆，也可查看或切换自动记忆；不同用户的数据互相隔离，群聊不可访问。
- 查看记忆和查看设置为只读操作；保存、更正、开关自动记忆需要确认，遗忘按破坏性操作确认。关闭自动记忆后不再隐式提取画像、生成每轮长期摘要或执行 enrich 长期学习，但明确要求记住的内容、短期上下文和 Daily Journal 仍正常工作。
...
## 陪伴房间 PWA 2026-08-28 11:43 +08:00

- 新增独立的 `/companion-room` 可安装页面，复用 QQ 私聊共处房间的同一运行时和状态文件；可开始专注、放松、共读、共看、共听，查看倒计时，暂停、继续、结束，记录内容进度并切换陪伴密度。
- 页面只操作 `COMPANION_ROOM_WEB_USER_ID` 绑定的单一 QQ 用户，不接受请求参数切换身份；继续复用 Web 管理会话、管理员写权限和严格同源校验。离线时保留应用壳与最近一次成功读取的房间状态，离线操作不会伪装成功。
- PWA 使用现有角色图片的裁切版本作为本地资产，不新增前端框架或第二套房间状态。配置、边界和验收见 [QQ 私聊共处房间开发文档](docs/qq-companion-room-2026-08-14.md)。
...
- 重启验收（2026-09-04 02:39 +08:00）：`restart-bot.cmd restart confirm` 返回 `healthy=True`，主进程 PID `14580`、post-reply worker PID `40856` 均为 Running；重启后的 `bot-runtime.err.log` 为空。重启后尚未产生新的普通用户请求，因此 `request-trace.ndjson` 和 `model-calls.ndjson` 中没有可用于证明新请求阶段块的样本；代码级真实用户探针仍确认 `1052258894` 为 `friend/普通朋友` 并选择 `guanxi/03.txt`。
...
- **回复出口拦截**：群聊和普通用户私聊发送前使用本地政治敏感词库快照；普通政治词仍要求命中现实政治语境，`unconditionalWords` 中的 8964、六四、八九、天安门事件、坦克人相关变体及补充领导人姓名命中即替换。管理员私聊豁免，角色扮演标记不能抵消无条件拦截（2026-09-06）。
...
维护记录：2026-08-27 09:42 +08:00，Bot 周期重启计划改为每天 09:30、21:30；本机任务已重新注册并实查两个触发器，下一次运行时间为 2026-08-27 21:30。
...
- 普通用户标准主回复支持动态 `MAIN_MODEL_<数字>_*` 槽位；槽位编号可跳号并可继续扩展，每次请求随机选择起点，当前请求内失败立即切换未尝试槽位，全部失败后进入 `AI_FALLBACK_*`（2026-08-31）。
- 旧 `API_BASE_URL`、`API_KEY`、`AI_MODEL`、`API_PROVIDER` 配置继续兼容；管理员、`normal_fast_reply`、图片和后台专用链路保持原模型选择。

## 普通用户主模型池权重（2026-09-01）

- 新增可选 `MAIN_MODEL_N_WEIGHT`；缺省权重为 `1`，当前 `.env.example` 将 `MAIN_MODEL_1_WEIGHT` 配为 `3`，其他示例槽位配为 `1`。
- 普通用户标准主回复按权重随机生成本次请求的无放回候选顺序；1 号槽位作为首选的概率是普通 1 倍槽位的 3 倍，失败切换、备用模型和管理员/快速/图片链路边界保持不变。
