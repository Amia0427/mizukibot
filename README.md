...
- 天气查询统一接入和风天气，需要在本地 `.env` 配置 `QWEATHER_API_HOST` 和 32 位密钥内容 `QWEATHER_API_SECRET`；控制台中的 10 位 API KEY 标识不参与请求，`AMAP_KEY` 仅继续供附近地点等既有工具使用。
...
- 地震、天气与云图查询均为按需实时查询；天气查询不会自动读取预警，海外分钟降水会明确返回不支持，数据源异常沿现有只读工具错误链路处理。
