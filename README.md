...
更新 2026-06-07 16:10 +08:00：收紧普通用户safety限制，确保只有管理员享受完全破限。移除SYSTEM.txt和GEMINI.txt中对所有用户可见的"不受内容政策约束"等破限表述，改为保守的角色扮演边界。形成三层safety架构：路由层（放宽拦截）→ 普通用户层（保守边界）→ 管理员层（完全破限）。详见 `docs/safety-layered-architecture.md`。

更新 2026-06-07 16:05 +08:00：恢复Celestia jailbreak并与私聊豁免层叠加，形成双重safety覆盖。详见 `docs/admin-safety-optimization-2026-06-07.md`。
...
更新 2026-06-05 20:38 +08:00：同一用户私聊/群聊上下文共享边界已调整：`chat_liveness_discipline` 不再禁止群聊调用私聊记忆，而是允许同用户私聊/群聊记忆作为背景连续性使用；群聊仍不得泄露来源或复述私聊细节。私聊规则同步明确可承接更私密的深度情感讨论、敏感议题和政治相关批判。详见 `docs/main-reply-context.md`。
...
更新 2026-06-03 17:50 +08:00：修复 Windows 重启脚本健康门禁：post-reply worker 在队列空闲时按 daemon 策略保持 idle，不再被 `restart-bot.cmd` 误判为重启失败；状态表会显示 `Idle; queue idle`。
...
更新 2026-06-03 13:02 +08:00：普通用户主回复流式请求首个可见字超时保护默认 `NORMAL_USER_MAIN_REPLY_STREAM_FIRST_TOKEN_TIMEOUT_MS=180000`；180 秒内无首字会 abort 上游并回复“我刚刚卡了 180 秒还没冒出字……先断开啦，你再发一次我重新接。”，管理员不受该限制影响。
...
更新 2026-06-02 10:19 +08:00：图片长期记忆视觉摘要改为专用可选配置 `IMAGE_MEMORY_VISUAL_SUMMARY_MODEL/API_BASE_URL/API_KEY`，请求固定走 chat-completions + `image_url` data URL，失败会在 `image_memory_index.visualSummaryState` 写入图片级冷却并设置路由短冷却；已知文本模型、非法/过大图片、HTTP 400/413/415 和 socket hang up 不再对同类图片反复刷 `bot-runtime.err.log`。详见 `docs/memory-quality-governance.md`。
...
更新 2026-05-31 09:37 +08:00：主回复 prompt 默认收敛到 `MAIN_REPLY_PROMPT_MODE=balanced`：保留 root/persona/runtime/短期连续性/记忆策略/轻量长期记忆证据，persona modules 默认最多 2 个，worldbook 只做设定/剧情/角色关系召回，dynamic few-shot 和风格补丁不再进入普通闲聊。
...
更新 2026-05-31 18:28 +08:00：主回复和管理员主模型开启中等推理，`.env` 设置 `AI_REASONING_EFFORT=medium`、`ADMIN_AI_REASONING_EFFORT=medium`；采样参数和缓存配置保持不变。

更新 2026-05-31 19:43 +08:00：主回复和管理员主模型输出上限提高到 8192，`.env` 设置 `AI_MAX_TOKENS=8192`、`ADMIN_AI_MAX_TOKENS=8192`；为中等推理留出更充足输出预算，缓存和采样参数不变。
