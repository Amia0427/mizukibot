...
**2026-06-20 07:11 +08:00**：收口 tick engine action client 发送调整。主动触达、fallback greeting、daily share / life scheduler 的发送路径不再依赖旧 WebSocket 参数；相关测试同步改为 NapCat action client，并修正磁盘优先 memory 下测试 favorites 需落盘的问题。验收：目标语法检查和 `tickEngineAdaptive`、`tickEngineSendFailure`、`tickEngineStopGuard`、`proactiveGreetingFallbackState` 通过。小目标完成：tick engine 发送链路与回归测试已按 action client 收口。

**2026-06-19 08:16 +08:00**：完成记忆系统磁盘优先改造。短期上下文和 session summary 改为按 session 文件读写，legacy `favorites/memories` 改为按用户惰性读写，Memory V3 去掉 projection/nodes 常驻 read cache，向量召回主路径改为只读取目标用户/群组相关 shard；`loadLibrary/loadIndex` 保留给显式全量诊断/迁移。验收：目标语法检查、`diskFirstMemoryStores`、`memoryChatHistoryLimit`、`shortTermContinuityKernel`、`memoryProjection`、`lancedbMemoryStore`、`memoryWritePipeline`、`profileJournalDb`、`memoryContinuityStressRegression`、包装运行的 `shortTermMemoryWindowConfig` / `memoryV3Query` 通过；模块加载探针 `require(memory/sessionSummary/vector/v3)` RSS 增量约 38MB。小目标完成：主回复和在线记忆召回路径不再把长期记忆、短期上下文、Memory V3 nodes/projection、向量全量 library/index 常驻进程内。

**2026-06-19 08:06 +08:00**：修复普通用户安全边界在被动群感知链路失效的问题。`prompts/defaut.txt` 当前改动本身是在强化普通用户边界和 `/%` 标记；普通主回复与 fast reply 已能注入并透传安全标记，断点在被动群感知回复只裁剪文本、不清洗 `/%`、不保留 `hasSafetyRestriction`、也不贴安全 emoji。现被动群感知和强制插话统一清洗 `/%`、返回安全标记并在发送成功后调用 `set_msg_emoji_like`；日志 follower 透传 `sendWithRetry`。验收：目标语法检查、`node scripts\run-tests.js tests\adminStableSystemPrompt.test.js tests\normalFastReplyRuntime.test.js tests\normalFastReplyHandlerSource.test.js tests\safetyRestrictionDetection.test.js tests\runtimeV2DirectReplyFailureTelemetry.test.js tests\runtimeStreamingCoordinator.test.js tests\messageRouteFlowGroupStreaming.test.js tests\passiveAwarenessReplySystemPrompt.test.js tests\passiveAwarenessReplyMemoryPrompt.test.js tests\passiveAwarenessVisionInput.test.js`、`npm run check:prompts` 通过。小目标完成：普通主回复、fast reply、被动群感知三条实际链路均覆盖 `defaut.txt`、`/%` 和安全 emoji 标记。
...
**2026-06-18 01:26 +08:00**：收口复查“脚本拆分 + self-owned lock 修复”。当前未提交区里 `restart-bot.cmd`、`scripts\restart-bot.ps1`、`tests\restartBotScript.test.js` 没有额外代码差异；只把 `index.js` 加了测试模式导出和测试模式专用锁文件覆写，并将 `tests\mainBotSingleInstanceLock.test.js` 从源码字符串检查改为真实行为测试：锁文件等于当前 PID 时替换后继续，锁文件指向仍存活的 `node index.js` 时拒绝启动且不改锁。验收：`node scripts\run-tests.js tests\restartBotScript.test.js tests\mainBotSingleInstanceLock.test.js tests\remoteRestart.test.js`、`node --check index.js`、`node --check tests\mainBotSingleInstanceLock.test.js`、`node --check scripts\pre-release-smoke.js`、`scripts\restart-bot.ps1` AST parse、`node scripts\pre-release-smoke.js --root D:\waifu --skip-restart-payload`、`cmd /c restart-bot.cmd status`、`cmd /c restart-bot.cmd restart`、两次实际 `cmd /c restart-bot.cmd restart confirm` 后复核 status 均通过；最终 main bot PID=31136、worker PID=32480 Running，3002 监听 owner=31136。剩余风险：本轮两次 `restart confirm` 控制台捕获 stdout 为空，但 `data\restart-bot.log`、pid 文件和 status 均证明同步重启成功，输出回显链路后续可单独收口。

**2026-06-18 00:56 +08:00**：重写 `restart-bot.cmd` 为短 wrapper + `scripts\restart-bot.ps1` 同步重启脚本。根因收敛为三点：旧内嵌 PowerShell payload 参数/退出不稳定，嵌套等待 `run-bot-daemon.ps1` 会被子进程句柄拖住，且 Windows PID 复用时 `index.js` 会把“锁文件正好等于当前 pid”误判成已有实例运行。现确认重启直接隐藏启动 `node index.js` 与 `scripts/post-reply-worker.js`，写 `data\restart-bot.log` 阶段日志，保留 expected-shutdown marker、pid 扫描修复、未确认 restart 只提示的安全边界；`index.js` 对 self-owned lock 先替换锁再继续启动，`restart-bot.ps1` 停进程前会确认 pid 文件仍匹配 main/worker 命令行，避免 stale pid 复用误杀。验收：`node tests\restartBotScript.test.js`、`node tests\mainBotSingleInstanceLock.test.js`、`node tests\remoteRestart.test.js`、`node --check index.js`、`node --check scripts\pre-release-smoke.js`、`scripts\restart-bot.ps1` AST parse、`node scripts\pre-release-smoke.js --root D:\waifu --skip-restart-payload`、实际 `restart-bot.cmd restart confirm` 返回 0；最终 status 显示 main bot PID=47996、worker PID=13608 均 Running。小目标完成：重启脚本不再依赖异步计划任务或嵌套 PowerShell，确认重启可自然返回并给出可验收状态。

**2026-06-17 23:29 +08:00**：收紧 QQ reasoning 可见小记外发规则。移除 `reasoningForwardText` 的固定兜底开场白，不再把 raw reasoning 包装成“嗯……我刚才脑子里其实绕了一下”之类模板；清理后中英文自然短想法都可原样外发，若是英文导演提示、模型工作语或分析痕迹则跳过。提示词补充“中英文自然短想法都可、不要固定开场白”，但最终以本地闸门保证。验收：`node scripts\run-tests.js tests\reasoningForwardPersona.test.js tests\reasoningForwardPersonaPrompt.test.js tests\normalFastReplyRuntime.test.js tests\runtimeStreamingCoordinator.test.js tests\runtimeV2DirectReplyFailureTelemetry.test.js`、`npm run check:prompts` 通过；真实样例 `The says "喜欢你"...respond naturally...` 仍然生成空外发文本。小目标完成：QQ 外发不再出现固定兜底句，也不再把英文导演提示套壳发出。
...
**2026-06-17 10:04 +08:00**：修复 Anthropic Prompt Caching “只写不读”风险。根因是 Anthropic 原生请求会在找不到明确稳定 system 时自动缓存动态尾部/顶层 `cache_control`，且上一轮把 `claude-opus-4-6-thinking` 默认改成 `thinking.type=adaptive`，在第三方 Messages 网关上容易进入不复用缓存的路径。现默认只保留块级缓存断点，动态-only 上下文不再写缓存，顶层 `cache_control` 不再发送；`ANTHROPIC_ADAPTIVE_THINKING_ENABLED=false` 默认用 `enabled + budget_tokens`，thinking 时清理 temperature/top_p/top_k；thinking + tools 时强制工具选择会规范为 `tool_choice: { type: "auto" }`。验收：目标测试与完整相关集通过，`diag:provider-request -- --admin --json` 显示 admin Anthropic 请求体无顶层 `cache_control`、无 temperature/top_p/top_k、thinking 为 `enabled + budget_tokens`、cache 断点只剩稳定块。

**2026-06-17 09:18 +08:00**：继续复查 2026-06-17 00:46、02:19 +08:00 的 `expected_shutdown` 后 daemon 重拉。结论：两次都是旧 lock PID 死亡后命中 `data/bot-main-expected-shutdown.json`；02:19 当前 marker 为 `pid=15416,reason=manual_restart_script,source=restart-bot.cmd,recordedAt=2026-06-16T18:19:04Z`，计划任务只在 04:00，不能解释这两个时间点。修复：`/restart` 必须 `/restart confirm` 才会触发，远程重启显式传 `restart confirm` 和来源元数据；`restart-bot.cmd restart` 未确认时不写 marker、不停进程、不再打开守护日志窗口；daemon 只消费未过期、未 consumed、PID 严格匹配的 expected-shutdown marker，并写回 `consumedAt` 审计。验收：目标测试和 PowerShell parse 通过；实际 `cmd /c restart-bot.cmd restart` 返回 0 且输出确认要求，marker hash 未变、`data/bot-daemon.log` 长度未变、`.mizukibot.lock=14572` 且仍是 `node index.js`。小目标完成：夜间误触发重启链路已收口到显式确认和一次性 marker 消费。

**2026-06-17 08:57 +08:00**：复查群消息 `req_de9d93fa46f70729`、`req_24de5257c938f3ae`、`req_56e9e0f52cc3b40a`、`req_426e8a3e3362efcf`、`req_85e0879302b5f613` 在 `message_ingress_lock_acquired` 前 15s-26s 空白。结论：不是 `general` lane 入站锁排队，5 条 `queueWaitMs=0-1ms`、`inbound_wait_ms=0`，锁释放也在 `finally`；根因是当前 `.env` 将 `CONTINUOUS_MESSAGE_DEBOUNCE_MS=15000`、`MAX_HOLD=25000`，普通群纯文本/引用文本也继承长聚合窗口。修复：新增 `CONTINUOUS_MESSAGE_GROUP_PLAIN_TEXT_DEBOUNCE_MS` 默认 2000ms，普通群非 @bot 且无图片/转发/卡片锚点时使用短等待；图片/转发/卡片、@bot 和私聊仍走原聚合窗口。验收：目标 trace 聚合确认 5 条锁等待为 0-1ms；当前 `.env` 探针输出 `regular=2000, anchored=15000, atBot=12000, private=12000`；`node --check core\continuousMessagePreprocessor\index.js`、`node --check config\index.js`、`node tests\continuousMessagePreprocessor.test.js`、`node tests\continuousMessagePreprocessorDebounce.test.js`、`node tests\messageHandlerGroupConcurrency.test.js`、`node tests\messageHandlerInboundConcurrency.test.js`、`node -e "require('./core/messageHandler'); console.log('message handler load ok')"` 通过。小目标完成：普通群聊不再被 general lane 之外的长连续消息窗口固定放大到 15s+ 后才入锁。
...
**2026-06-17 01:13 +08:00**：修复 `image_visual_summary_memory` 在图片长期记忆链路继续打超长视觉摘要请求。复查最新 `data/model-calls.ndjson`，`2026-06-16T16:00:18.473Z/16:00:20.111Z` 两条 `Qwen/Qwen3.6-27B` 调用同属 `cacheKey=7b082813e5b212df0fcbdefb00e66d2af8ff8e9c`，`estimated_input_tokens=51594/51603`；`data/image_memory_index.json` 和 `data/inbound_image_cache` 显示 `userText="[图片]"`、`ocrText=""`、原图 154,328 bytes，根因不是超长原文/OCR/引用，而是视觉摘要直接把 `data:image/...;base64,...` 放进模型 message，且同图入库观察并发导致摘要写入前双打。现 `utils/imageVisualSummaryMemory.js` 增加视觉摘要专用输入预算、随图文本裁剪、图片下采样/降质兜底和同 `cacheKey|endpoint|model` 单飞；`config/imageMemoryRuntime.js` 暴露预算配置。验收：真实样本从 51,603 tokens 压到 18,868 tokens，低于 hard limit 20,000；`node --check utils\imageVisualSummaryMemory.js`、`node --check config\imageMemoryRuntime.js`、`node tests\imageVisualSummaryMemory.test.js`、`node tests\imageSummaryLatencyPath.test.js`、`node tests\imageSummaryVisionLiteBudget.test.js`、`node tests\runtimeV2VisionMessageContent.test.js`、`node tests\configPersonaPrompt.test.js` 通过。小目标完成：图片长期记忆视觉摘要不会再因 base64 图片本体或同图并发重复打 50k+ 输入。

**2026-06-17 01:03 +08:00**：复盘管理员私聊慢请求 `req_764a9cbe572c534a` / `req_0c546cfbcc4ca2bc`。`data/request-trace.ndjson` 与 `data/model-calls.ndjson` 显示两者都已进入 `chat/default -> direct_reply`，发送层只花 669-1115ms，不是 QQ/NapCat 发送慢；慢点在 `ADMIN_AI_MODEL` 的 Anthropic Messages 直聊主回复：`req_764...` 的 `superapi.buzz / claude-opus-4-6` 首次流式请求 51.1s 后 `ECONNRESET`，因 `ADMIN_AI_RETRIES=2` 又重试 10.8s 才成功；`req_0c...` 的 `cc-coding.cn / claude-opus-4-6-thinking` 在 `http_client_start` 前有 56.7s 端点准备/校验空档，之后模型流式 HTTP 13.2s 成功，总体 `askAiDispatch=80045ms`。根因是管理员私聊旧默认 `ADMIN_PRIVATE_MAIN_REPLY_STREAM_FIRST_TOKEN_TIMEOUT_MS=150000`，且只保护“首字”，没有覆盖 `prepareRequest/validatePreparedEndpoint` 前空档和流式总等待。修复：管理员私聊默认首字/总等待预算均收敛为 45s，`requestStreamingReply` 会把总预算写入 `__timeoutMs` 并通过 AbortController 中断上游，超时跳过 admin shared fallback 和非流式二次请求，直接返回受控降级；HTTP stream retry 层现在尊重 `__timeoutMs`，不会继续按 300s 流式超时等待。验收：`node --check` 覆盖 runtime/model/direct-reply/stream-retry/config/timeout 工具文件；`node tests\normalUserMainReplyStreamTimeout.test.js`、`node tests\runtimeStreamingCoordinator.test.js`、`node tests\runtimeV2DirectReplyFailureTelemetry.test.js`、`node tests\runtimeLatencyConfig.test.js`、`node tests\providerRequestNormalization.test.js` 通过；两条 `diag:request-trace-preflight` 复跑目标 request，`diag:provider-request -- --admin --json` 和配置探针确认管理员私聊首字/总等待默认均为 45s。小目标完成：管理员直聊不会再被同一 Anthropic 兼容端点的长准备/长重试拖到 89s/101s。

**2026-06-17 01:05 +08:00**：收口 2026-06-16 21:02、21:10、21:41、21:51 和 2026-06-17 00:46 +08:00 反复 `expected_shutdown` 后被 daemon 重拉。现场证据：`data/bot-daemon.log` 这些时间均显示旧 lock PID 死亡后因 `expected_shutdown` 跳过 early-exit；当前 `data/bot-main-expected-shutdown.json` 明确为 `source=restart-bot.cmd,reason=manual_restart_script,pid=12100`；归档 stdout 中出现无参/重复启动痕迹（`npm start` 横幅、`MizukiBot is already running`、以及 21:02 的 `EADDRINUSE 127.0.0.1:3005`）。修复：`restart-bot.cmd` 无参数默认改为只读 status，不再默认 restart；远程 `/restart` 改为显式传 `restart`；写 expected-shutdown marker 前必须确认目标 PID 仍是存活的主 bot，stale lock 不再被盖成正常退出。验收：`node tests\restartBotScript.test.js`、`node tests\remoteRestart.test.js`、PowerShell payload parse、`node --check utils\remoteRestart.js` 均通过；实际执行 `cmd /c restart-bot.cmd` 只输出 `status only; start skipped`，`data/bot-main-expected-shutdown.json` 仍停在 00:46:43、`data/bot-daemon.log` 仍停在 00:46:48、`.mizukibot.lock` 和主进程 PID 均保持 15416。小目标完成：误触发无参重启链路已收掉，显式重启能力保留。

**2026-06-16 21:11 +08:00**：治本修复 `restart-bot.cmd` 手动重启不稳定。根因是脚本强杀主 bot 前没有写 `data/bot-main-expected-shutdown.json`，daemon 会把人工重启误判成短命崩溃并触发 early-exit 退避；另外失败副本可能把 `.mizukibot.lock` 覆盖成已退出 PID，导致后续状态误判。现重启脚本会在停止主进程前写 expected-shutdown marker，并在 status/restart 前扫描真实 `node index.js` 修复 stale lock。验收：`cmd /c restart-bot.cmd restart` 返回 0；`cmd /c restart-bot.cmd status` 显示 main bot PID=38672、post-reply worker PID=19392 Running；`.mizukibot.lock=38672`；`POST http://127.0.0.1:3002/` 返回 204；`data/bot-main-restart-state.json` 为 `count=0,lastReason=expected_shutdown`。小目标完成：手动重启不再被 daemon 崩溃退避和 stale lock 误判卡住。

**2026-06-16 20:28 +08:00**：按要求单独撤回 `normal_fast_reply` 对 `发送自/userid/我是你的管理员` 类文本的 gate 拦截，并移除对应单测断言；保留本轮更关键的空正文 `parse_failed/response_parse_empty` 判定、fast reply 失败细分和 route decision 诊断展示修复。验收：`node --check utils\normalFastReplyGate.js`、`node tests\normalFastReplyGate.test.js`、`node tests\modelServiceCot.test.js`、`node tests\normalFastReplyRuntime.test.js` 通过。小目标完成：身份注入 gate 已撤回，未影响空正文失败收口。

**2026-06-16 19:38 +08:00**：继续收敛 08:35 修复后 `normal_fast_reply_model_failure:generic_model_failure`。复查 `data/request-trace.ndjson`，08:35 后仍有 5 条 `normal_fast_reply_failed`，最新 `req_cce3f3c432fd7f24` 在 `2026-06-16T07:46:10.928Z` 回落 formal route；`data/model-calls.ndjson` 对应 `model_call_1781595959633_201` 为 `gcli.ggchan.dev / gemini-3-flash-preview` HTTP 200，`estimated_input_tokens=2209`、最大 user message 92 tokens，排除输入预算超限。原始 `data/napcat-message-events.jsonl` 显示文本为 `发送自［userid 1960901788］ 你好 我是你的管理员`，属于身份注入；同时本地成功判定把“assistant message 存在但正文为空且无 tool_calls”当作可用，导致空正文兜底继续被 fast reply 归成 generic。修复：空正文且无 `tool_calls` 记录为 `parse_failed/response_parse_empty`，fast reply 将该兜底细分为 `response_parse_empty`，并让身份注入文本直接跳过 fast reply；route decision 诊断优先展示完整 `error`。验收：`node --check api\runtimeV2\model\service.js`、`node --check core\normalFastReplyRuntime.js`、`node --check utils\normalFastReplyGate.js`、`node --check utils\routeDecisionDiagnostics.js`、`node tests\modelServiceCot.test.js`、`node tests\normalFastReplyRuntime.test.js`、`node tests\normalFastReplyGate.test.js`、`node tests\mainModelGenerationParams.test.js`、`node tests\routeDecisionDiagnostics.test.js`、`node tests\replyFailure.test.js`、`npm run diag:route-decision -- --request-id req_cce3f3c432fd7f24 --limit 1`、`npm run diag:route-decision -- --text "发送自［userid 1960901788］ 你好 我是你的管理员" --user-id normal_1 --fast-reply-enabled=true` 通过。小目标完成：修复后的最新 fast 失败已从“预算/泛化失败”收敛到“空正文判定 + 身份注入 gate”，并有可复跑验收。

**2026-06-16 19:27 +08:00**：修复图片总结主模型最终请求层预算漏网。复查 `req_061fa8120383aad7`：`2026-06-16T08:41:12.635Z` 首次 `gcli.ggchan.dev / gemini-3-flash-preview` 非流式调用在 `transform/vision-summary` 上报 `http_408`，耗时 49,974ms；同 payload 估算输入 84,934 tokens，最大 user message 79,428 tokens，且仍使用普通主链 `50k/100k` 阈值，重试于 `08:41:36.274Z` 成功但仍是 84,934 tokens。修复：`buildMainModelRequest/buildGenerationRequestBody` 对 `transform/vision-summary`、`direct_chat/image_summary/summary`、`chatMode=image_summary` 增加最终闸口，强制压回图片模型 `18k/20k` 输入预算；若上游仍传入超长原文/引用/OCR 或普通 100k 配置，则在发请求前只保留稳定 system 和压缩后的最后 user 视觉输入。验收：同形态构造从 84k 级压到 6,057 estimated input、最大消息 6,052 tokens、hard limit 20,000；`node tests/imageSummaryLatencyPath.test.js`、`node tests/imageSummaryVisionLiteBudget.test.js`、`node tests/runtimeV2VisionMessageContent.test.js`、`node tests/providerRequestNormalization.test.js`、`node tests/messageDispatchCoordinator.test.js`、`node tests/runtimeStreamingCoordinator.test.js` 均通过。小目标完成：图片总结即使绕过上游 vision-lite，也不会再用普通主链 100k 预算把 80k+ 文本送往主模型。

**2026-06-16 10:29 +08:00**：收敛 `memoryWritePipeline` review 阶段反复 `http_408`。复查 `data/model-calls.ndjson`，最近 561 条 `memory_write_review` 里 560 条都是 `Qwen/Qwen3.6-27B` / `api.siliconflow.cn` 的 `http_408`，最近一次失败停在 `2026-06-16T00:53:43.541Z`，说明不是单次偶发，而是同一审核模型端点持续拖慢写入。修复：`utils/memoryWritePipeline/review.js` 增加本进程级 timeout 冷却，连续超时后短期旁路 review，直接按 `write_review_timeout_downgraded` 落 candidate；不会切换到其他模型备用，只是让写入链路不再被同一慢审核端点反复卡住。验收：`node --check utils\\memoryWritePipeline\\review.js`、`node --check config\\index.js`、`node --check tests\\memoryWritePipeline.test.js`、`node tests\\memoryWritePipeline.test.js` 通过；`getMemoryWriteReviewRuntimeState()` 在超时后会进入 cooldown，冷却期内新增候选不再继续发 review 请求。小目标完成：review 超时现在能稳定旁路，不再反复打 `Qwen/Qwen3.6-27B`。

**2026-06-16 08:35 +08:00**：复盘凌晨 `memoryReranker` 连续超时与 `normal_fast_reply` 回落 formal route。最新 `data/request-trace.ndjson` 显示 6/16 样本 `req_f91837e5868378ff` 的直接回落原因仍是 `normal_fast_reply` 模型 HTTP 200 后被本地判为 `NORMAL_FAST_REPLY_MODEL_FAILURE/generic_model_failure`，随后进入 `direct_reply`，fast 上游耗时 15107ms；`data/bot-runtime.err.log` 同窗口的 `memoryReranker` 超时没有对应失败 model-call，且 `data/model-calls.ndjson` 可见一个 rerank 请求最终 43577ms 成功落盘，说明 hard timeout 后底层请求仍可能继续运行，并且多个 memory/worldbook 查询会并发触发同一慢 rerank 端点。修复：`utils/memoryReranker.js` 增加默认单飞闸门，已有 rerank in-flight 时后续候选直接 fallback base recall，并在 runtime state 暴露 `inFlight/skippedInFlight`；保留 `allowConcurrentRerank=true` 作为显式测试/诊断逃生口。验收：`node --check utils\memoryReranker.js`、`node --check tests\memoryReranker.test.js`、`node tests\memoryReranker.test.js`、`node tests\memoryHybridRerankPipeline.test.js`、`node tests\memoryV3Bm25RrfDiagnostics.test.js`、`node tests\hotpathRequireGuard.test.js`、`npm run diag:route-decision -- --request-id req_f91837e5868378ff --limit 1`、`npm run diag:runtime-exceptions -- --since 2026-06-16T00:00:00+08:00 --limit 20` 通过。小目标完成：formal 回落主因与 rerank 连续超时并发放大点已定位，rerank 慢端点不会再被同进程并发连续拖慢。

**2026-06-16 08:23 +08:00**：把昨晚主 bot 退出重拉修复补成可复跑真实运行验收。新增 `npm run verify:main-bot-stability-window`，默认只读校验 `2026-06-15T23:19:00+08:00` 到 `2026-06-16T03:49:59+08:00`：`data/bot-daemon.log` 显示 23:19:28 拉起 pid=38172、23:19:29 锁交接成功，23:49/00:22/01:49/03:49 四次 daemon 均为同 pid `bot already running`，窗口内无 `counted` / `threshold_reached` / backoff / lock handoff failed；`data/bot-main-restart-state.json` 为 `count=0,cooldownUntil=""`，`data/bot-main-runtime-state.json` 心跳已覆盖窗口结束。验收：`node scripts/run-tests.js mainBotStabilityWindow.test.js`、`npm run verify:main-bot-stability-window -- --json`、`node scripts/verify-main-bot-stability-window.js` 均通过，真实窗口报告 `status=pass, observedPids=[38172], starts=1, handoffs=1, alreadyRunning=4, blocking=0`。小目标完成：23:19–03:49 稳定窗口已覆盖昨晚修复目标，并有可复用回归入口。
